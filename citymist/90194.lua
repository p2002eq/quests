-- Lord Rak`Ashiir for shaman epic (nice version)
function event_trade(e)
	local item_lib = require("items");
	if e.other:GetFactionLevel(e.other:CharacterID(), e.self:GetID(), e.other:GetRace(), e.other:GetClass(), e.other:GetDeity(), 342, e.self) == 1 then
		if(item_lib.check_turn_in(e.self, e.trade, {item1 = 1673})) then -- Child's Tear
			...
			eq.spawn2(90196,0,0,e.self:GetX(),e.self:GetY(),e.self:GetZ(),e.self:GetHeading()); -- Lord Rak`Ashiir
			eq.depop();
		end
	else
