...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        local zoneGlobal = "POFIRE-" .. tostring(e.other:GuildID());
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(217, tonumber(instanceId), -1395,1205,-178,128);
        else
...
local static_named = {217407};	--disable these spawnpoints since they don't share other trash NPCS (also are included above in disabled instance)

function event_spawn(e)
    local instance_id = eq.get_zone_instance_id();
    if(instance_id ~= 0) then
		local mysql = require("luasql_ext");
		for id,name in mysql.rows(con,"SELECT id from npc_types WHERE id >= 217000 and id < 218000 and disable_instance = 1") do
