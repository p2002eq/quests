...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Oh, hey there.. How ya doing? My name's Den. I work for ol' [" .. eq.say_link("Rohand") .. "] here, unloading the [" .. eq.say_link("shipments") .. "] that come in from overseas.");
	elseif(e.message:findi("rohand")) then
		e.self:Say("Ol' Captain Rohand is the owner of the Mermaid's Lure. He trades goods with sailors and sea [" .. eq.say_link("merchants") .. "]. I think that old man was even born at sea.. And he's got more stories than fish have scales.");
	elseif(e.message:findi("shipments") or e.message:findi("merchants")) then
		e.self:Say("Yeah.. We get merchants and traders with goods from here to [" .. eq.say_link("Odus") .. "].");	
