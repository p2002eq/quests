...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 31769, item2 = 31768})) then -- Hollow palm tea & Metal Strainer
		...
		e.other:QuestReward(e.self,0,0,0,0,31771); -- Runed Metal strainer
	end
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 31772})) then -- Spiced Grimroot Tea
		...
		e.other:QuestReward(e.self,0,0,0,0,31776); -- Ahlams Approval
	end
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 31776, item2 = 31771})) then -- Ahlams Approval & Runed Metal strainer
		e.self:Say("Here is the Acrylia Handled Strainer.  Use it with regular water to replace the crystal clear water.");
		e.other:QuestReward(e.self,0,0,0,0,31777); -- Acrylia Handled Strainer
	end
