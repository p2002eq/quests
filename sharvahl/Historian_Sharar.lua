function event_say(e)
	...
	elseif(e.message:findi("yes")) then
		e.self:Say("You have a braver spirit than mine. friend. but I'm glad that someone is available to assist me.  Take care and keep alert.  The Khati Sha seem to be very [" .. eq.say_link("troubled") .. "] by something of late.");
		e.other:SummonItem(9061); -- Collection of Spirit Lore
	elseif(e.message:findi("troubled")) then
		e.self:Say("They have been researching the lore of elder spirits and their relationship to this realm. I really do not have much more information than that. I can only speculate that it has something to do with a disturbance in the spiritual realm. Perhaps... well... I cannot say for sure. If the Animist finds that he can trust you, perhaps you can learn more. I honestly have nothing more to offer you on the subject.");
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 8466})) then -- Ahoms Journal
		...
		e.other:SummonItem(4461); -- Earring of the Owlbear
		e.other:QuestReward(e.self,0,0,0,0,8467); -- Note for General Avilah
	end
