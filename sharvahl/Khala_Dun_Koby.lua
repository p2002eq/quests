...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 29803})) then -- A Shield Repair Order
		...
		e.other:QuestReward(e.self,0,0,0,0,29804); -- Kobys Dented Shield
	end
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 29805})) then -- Kobys Immaculate Shield
		e.self:Say("Tip-top work! Barkhem has done well by himself choosing you for an apprentice.");
		e.other:QuestReward(e.self,0,0,0,0,29806); -- Kobys work order
	end
