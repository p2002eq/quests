...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 14587, item2 = 14587, item3 = 14587, item4 = 14587})) then -- 4x Rhino Beetle Eye
		...
		e.other:QuestReward(e.self,3,3,0,0,0,1000); -- Just EXP and Coin
	end
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 29803})) then -- A Shield Repair Order
		e.self:Emote("smiles warmly at you,");
		...
		e.other:QuestReward(e.self,0,0,0,0,29813); -- Bokhs Dented Shield
	end
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 29814})) then -- Bokhs Immaculate Shield
		e.self:Say("Well done, shiny enough to eat off of. My compliments to you " .. e.other:GetName() .. ", Barkhem would be most pleased.");
		e.other:QuestReward(e.self,0,0,0,0,29815); -- Bokhs work order
	end
