...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 5960})) then -- Note for Jangle
		...
		e.other:QuestReward(e.self,0,0,0,0,17612); -- Jangles Bag
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 5961})) then -- Bag of Owlbear Guts
		...
		e.other:QuestReward(e.self,0,0,0,0,5962,1000); -- Note from Jangle
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 5963})) then -- Gawfeds Request
		...
		e.other:QuestReward(e.self,0,0,0,0,5965,1000); -- Bag of Lute Strings
	end
