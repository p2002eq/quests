function event_say(e)
	...
	elseif(e.message:findi("shard")) then
		e.self:Say("Searching for that blasted emerald shard, are you? That thing has done nothing but bring me bad luck!! I would gladly give it to you if you would do a [" .. eq.say_link("small favor") .. "] for me.");
	elseif(e.message:findi("small favor")) then
		if(e.other:GetFaction(e.self) < 5) then
			...
			if(eq.get_entity_list():IsMobSpawnedByNpcTypeID(33146) == false) then
				 eq.spawn2(33146,0.0, 0, 893.00, 664.00, -34.73, 0); -- Spawn Slaythe
			end
...
function event_trade(e)
	...
	if((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 12192}))) then -- Froglok Leg
		e.self:Say("So I see you have defeated Slaythe. I shall sleep much better now that he is gone. I placed the emerald shard in one of the Highkeep strongboxes in Highpass. Give the bank clerk's assistant, Kiolna this key. There is a [" .. eq.say_link("second piece of the gem") .. "] which I hid near Kelethin. You will need it to complete the gem. I hope it brings you better luck than I.");
