...
function event_say(e)
	...
	elseif e.message:findi("purpose") then
		e.self:Say("The teaching in the city is that Katta feigned his death to blame it on Seru. When in fact Seru actually poisoned Katta. The crusade that Seru leads is false and must be stopped. We must gain access to Seru's [" .. eq.say_link("chambers") .. "] and destroy him and show the city the truth.");
	...
	elseif e.message:findi("action") then
		e.self:Say("You must go see Lcea. She was Tsaph Katta's closest advisor. This matter has escalated greatly and must be placed in her hands. Ask her about the [" .. eq.say_link("Arx Key") .. "] she will no doubt ask in your service retrieving it. I salute your service to the city and wish you well.");
	elseif e.message:findi("praesertum") then -- This text is from where?
		e.self:Say("The four Praesertum can be found in the center of the city. Each has their own building, easily identified by the symbol above the main entrance. Stay clear of the Arx Seru, the central building. This is the resting place of [" .. eq.say_link("Seru") .. "], who does not allow anyone but the Praesertum to disturb him. To venture inside is death for all that are not invited.");
	...
	elseif e.message:findi("help") then
		e.self:Say(string.format("Excellent. Find [" .. eq.say_link("Claudius") .. "] and give that to him- it's gibberish but he and his new friends won't know that now will they? We have been watching someone named Vahn enter the city under the guise of a trader. We know he works for Seru, but we were not sure what he was doing here, so we let him be- trouble was he kept giving us the slip passing through the tunnels. Looks now like we may have found a connection. The plan sounds complicated and I don't want to confuse you, but if you stick with me it will all go just fine. You [" .. eq.say_link("with you",false,"with me") .. "] so far, %s?", e.other:GetName()));
	elseif e.message:findi("with you") then
	...
	elseif e.message:findi("claudius") then
...
function event_trade(e)
	...
	elseif (item_lib.check_turn_in(e.self, e.trade, { item1 = 29858, item2 = 29889 })) then -- Etched Earring of Veracity and Full Satchel
		e.self:Say("According to these reports Seru resides in a building called the Arx Seru. It is the large complex in the center of the four Praesertum Consillium. We must take [" .. eq.say_link("action") .. "] now. Do not underestimate the power of Sanctus Seru for what they do they feel is right.");
	...
	elseif (item_lib.check_turn_in(e.self, e.trade, { item1 = 29844 })) then -- Dossier: Case 3463827
		e.self:Emote("reads through the dossier and then looks at you, eyebrow raised curiously, 'look at the mess you have gotten yourself in the middle of,' he says with a chuckle, while beginning to jot a note. 'Here take this and...' he stops midsentence. 'I guess I haven't even given you the chance to say whether or not you want to [" .. eq.say_link("") .. "]help. Would you like to see this through to the end?'");
