...
function event_say(e)
    ...
    if(e.message:findi("Valdanov Zevfeer") and tonumber(qglobals.first_badge_nathyn) == 1) then
        e.self:Say("Valdanov Zevfeer was a talented alchemist from Shadow Haven with whom I had many dealings. He would bring me components for my enchantment research from all the various environments here on Luclin. Since Valdanov had been such a valuable aid to me in the past I decided to return the favor and allowed him residence within my abode and helped to further fund his research as long as he assisted me in my own enchantment studies. He has since left to continue to pursue his own interests and I have not heard from him in [" .. eq.say_link("quite some time") .. "].");
    elseif(e.message:findi("quite some time") and tonumber(qglobals.first_badge_nathyn) == 1) then
        e.self:Say("Valdanov departed many years ago. He had become dangerously obsessive with his research of vampyre physiology and the effects of various alchemical formulae on the undead creatures. His obsession began when a traveler arrived in Katta Castellum speaking of a Vah Shir vampyre. Valdanov began leaving the safety of the city frequently in search of this undead Vah Shir and for questionable alchemical reagents with which he insisted on testing on [" .. eq.say_link("captured vampyres") .. "] being studied by the Magus Conlegium.");
...
function event_trade(e)
    ...
    if(item_lib.check_turn_in(e.self, e.trade, {item1 = 31752})) then -- Katta Castellum Badge of Service (*Katta Castellum Badge of Service)
        e.self:Say("Ahh i see you're assisting Phenic. Perhaps looking into more information on [" .. eq.say_link("Valdanov Zevfeer") .. "]?");
