...
function event_say(e)
    if(e.message:findi("hail")) then
        e.self:Say("Greetings friend! Pull up a stool and order some of the finest ale in all of Luclin. If it's a [" .. eq.say_link("tale or two") .. "] you're lookin to hear as well I would suggest staying away from those two fools Arbogast and Miller. They're full of more hot air than a red dragon!");
    elseif(e.message:findi("tale or two")) then
        e.self:Say("I could tell you some interesting stories. I spent many years serving as a scout in the Validus Custodus. I've seen places and things that'll cause you to loose sleep for many days. One of the most facinating things I've seen however was while I was out on patrol near the [" .. eq.say_link("Acrylia Caverns") .. "].");
    elseif(e.message:findi("acrylia caverns")) then
        e.self:Say("The Acrylia Caverns are teeming with grimlings that tend to be a bit more evolved than your run of the mill dirtballs you find living in villages elsewhere on luclin. I was with a small patrol of scouts watching for any indication of the Acrylia Caverns Grimlings sending aid to the grimlings in Tenebrous Mountains during one of the Validus Custodus campains to rid the tenebrous mines of the little buggers. What we [" .. eq.say_link("discovered") .. "] was something quite different.");
    elseif(e.message:findi("discovered")) then
        e.self:Say("We spotted a small group of grimlings leaving the caverns hauling a cart full of acrylia ore. The strange thing was. this acrylia was darker than normal and instead of the normal golden hued veins it was covered with veins blacker than any darkness I've ever seen. I shadowed these grimlings to a grimling village some distance from the caverns where they exchanged some of this [" .. eq.say_link("black acrylia") .. "] with others of their kind.");
    elseif(e.message:findi("black acrylia")) then
        e.self:Say("Seems the grimlings use this rare acrylia ore to craft fierce weapons that not only cut or bash but strangle the spirits of those they harm. I observed some of their [" .. eq.say_link("method for crafting") .. "] the black acrylia but not being knowledgeable of the smithing arts myself and the distance at which I had to observe made the details a bit obscure.");
    elseif(e.message:findi("method for crafting")) then
        e.self:Say("I explained my observations to one of our master smiths here in Katta Castellum and he has since successfully worked the ore. The difficulty of obtaining the [" .. eq.say_link("necessary supplies") .. "] for crafting the black acrylia however prevents the Validus Custodus and Magus Conlegium from doing extensive research of the strange ore.");
    elseif(e.message:findi("necessary supplies")) then
        e.self:Say("We have discovered that the grimlings use a special Black Acrylia Temper when crafting sheets of black acrylia and then again when crafting those sheets into weapons with the appropriate weapon molds. I saw this substance during my observations and after many attempts we seem to have discovered its [" .. eq.say_link("ingredients") .. "].");
...
function event_signal(e)
    if(e.signal == 1) then
        e.self:Say("It translates to 'Gift of the Dark' you fool!");
        eq.signal(160112,3,2500); -- Arbogast
    end
end