...
function event_say(e)
    if(e.message:findi("Hail")) then
        e.self:Say("Greetings stranger. It's nice to have visitors of the living sort. Seems like my work here is never done. When I'm not having to dig fresh graves and keep the mausoleum clean I'm having to refill old graves that have been dug up by the [" .. eq.say_link("restless dead") .. "] or them disrespectful [" .. eq.say_link("grave robbers") .. "]!");
    elseif(e.message:findi("restless dead")) then
        e.self:Say("That's right. We've got our fair share of undead lurking around our fine city. Most of the sorry souls aren't as troublesome as the vampyres and we've even got a few [" .. eq.say_link("friendly spirits") .. "] right here in this very mausoleum.");
    elseif(e.message:findi("friendly spirits")) then
        e.self:Say("Every so often when Norrath is near its zenith a group of spirits gathers at the podium and chairs over there. Seems they were some of the original Loyalists that arrived here on Luclin and helped build our great city of Katta Castellum. Their bodies are at rest in the stone crypts along these mausoleum walls and guarded by the [" .. eq.say_link("gargoyle warders") .. "]. Yet even in death their spirits convene to discuss the condition of Katta Castellum and the teachings of Tsaph Katta.");
    elseif(e.message:findi("gargoyle warders")) then
        e.self:Say("The gargoyles that guard the residents of this mausoleum are quite fascinating. They were carved late in the construction of Katta Castellum by a very talented dwarven stone worker. Gundle Chislebeard. Gundle used a type of stone found in the very cliffs Katta Castellum is built upon. At first the gargoyles were nothing more than impressive statues but then over time they began to [" .. eq.say_link("change") .. "].");
    ...
    elseif(e.message:findi("list of belongings")) then
        ...
        e.other:SummonItem(18353); -- List of Stolen Items
    end
end

function event_trade(e)
    local item_lib = require("items");
    if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10670, item2 = 10673, item3 = 10672, item4 = 10671})) then -- Aeridia Family Signet Ring, Tarnished Copper Runed Wand, Faded Silver Embroidered Robe & Sealed Golden Scroll Tube
        ...
        eq.spawn2(160473,0,0,-1702,-633,11,4);
    ...
    elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 10674, item2 = 10675, item3 = 10676, item4 = 10677})) then -- Ancient Ornate Combine Dagger, Talikar Family Signet Ring, Faded Portrait of a Lady & Antique Platinum Medal
        ...
        e.other:Faction(284,-1); -- Seru
        e.other:Faction(298,-1); -- Shoulders of Seru
        eq.spawn2(160474,0,0,-1730,-568,11,200);
    ...
    elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 10678, item2 = 10679, item3 = 10680, item4 = 10681})) then -- Gold Embroidered Kilt, Silver Embroidered Tabard, Ancient Ceremonial Warhammer & Old Sealed Medicine Pouch
        ...
        e.other:Faction(284,-1); -- Seru
        e.other:Faction(298,-1); -- Shoulders of Seru
        eq.spawn2(160475,0,0,-1667,-559,11,260);
