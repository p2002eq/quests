function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Furless one wants something with snowfang? You here to help with [" .. eq.say_link("curse") .. "]?");
	elseif(e.message:findi("curse")) then
		...
		e.self:Say("Some days ago we found flat, strong, stretchy thing. Many scribbles, many marks on it. Ever since fish not come to us... Rest of the snowfang think this is bad magic. To not believe this is foolish. But I not able to just get rid of bad [" .. eq.say_link("magic") .. "].");
	elseif(e.message:findi("magic")) then
	    e.self:Say("To cast away is not something we can do. Snowfang believe bad magic thrown away just make worse magic. Only way to get rid of badness is for friend to bring gift [" .. eq.say_link("offering") .. "]. Offering that means much to snowfang. If that is done magic is broken by the giving.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 30295,item2 = 30296})) then -- toolset Delivery (kael)
		...
		eq.spawn2(110228,0,0,2000,1000,-31.9,222.4,1); --spawn an enraged walrus
	end
