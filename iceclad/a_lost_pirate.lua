function event_say(e)
	if(e.message:findi("hail")) then
		...
		e.self:Say("Ya ain't lost out here too, are ya?");
	elseif(e.message:findi("not lost")) then
		e.self:Emote("grins and stares through you with a mad gleam in his eye.");
		e.self:Say("Hehehe. Yah, yer lost aren't ya. I can tell from the crazy sound o' yer voice. Too bad we ain't got me map. The map would lead us straight back ta camp. Yah, that it would, boyo.");
	elseif(e.message:findi("map")) then
	    e.self:Emote("nods.");
		e.self:Say("After we got ourselves stuck on this island I got it in me head to draw up a map so none o' the crew would ever get lost. So I set myself to explorin' and drawin'. Now the map is gone. Too bad, too, cause it was a map o' the whole place, not just the first island, but a bunch o' parts.");
