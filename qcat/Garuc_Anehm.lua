function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Say("Welcome to the Shrine of Bertoxxulous! You appear to be [" .. eq.say_link("new to the shrine") .. "]. I do not remember seeing you around.");
	elseif(e.message:findi("new to the shrine")) then
		e.self:Say("New blood to spread Bertoxxulous' disease? We presently need the help of a [" .. eq.say_link("young shadowknight") .. "]. There are certain deeds which must be carried out.");
	elseif(e.message:findi("young shadowknight")) then
		e.self:Say("Then let your supreme task be the extermination of the Priests of Life. Know that they threaten the will of our Lord Bertoxxulous. For every fallen paladin and cleric of the Temple of Life. you shall rise within our ranks.  So... do you [" .. eq.say_link("loathe paladins") .. "]?");
	elseif(e.message:findi("loathe paladins.")) then
		e.self:Say("As do we all. The paladins of Qeynos come from either the Hall of Thunder or the Temple of Life. We have put a bounty upon their heads. Destroy these knights and return either the Order of Thunder or the prayer beads of these inferior men. There has even been a report from [" .. eq.say_link("Commander Kane") .. "].");
	elseif(e.message:findi("commander kane")) then
		e.self:Say("Kane Bayle is the Commander of the Qeynos Guard. He has found it advantageous to befriend the Bloodsabers. We share a [" .. eq.say_link("common goal") .. "]. He assists us in many ways. He has some new information to reveal.  Go and tell him you are a Bloodsaber. Do not speak with him while he is on duty!!");
