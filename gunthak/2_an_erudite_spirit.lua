...
function event_trade(e)
    ...
    if(qglobals["Millius_Paladin"] == "2") then
        if(item_lib.check_turn_in(e.self, e.trade, {item1 = 359027})) then -- Black Skull Necklace
            ...
            e.other:QuestReward(e.self,0,0,0,0,359028,eq.ExpHelper(45, 5)); -- Glowing Black Skull Necklace
            eq.delete_global("Millius_Paladin");
            eq.depop();
            reset_spawn();
        end
    else
        e.self:Emote("recoils from you. A whispering moan echoes towards you 'You are not the one who summoned me. . .'")
    end
