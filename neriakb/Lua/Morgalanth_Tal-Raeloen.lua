function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Say("Who dares to address me while I am clearly busy reviewing my daily research. I sure hope this is important " .. e.other:GetName() .. ", I do not have time to waste on just any initate of House Spurned. Not a single day goes by that I do not come face to face with cannon fodder that is not even worthy of a headstone in our burial grounds. If you wish to [prove yourself] then I might be in a generous mood.");
	end
	if(e.message:findi("prove myself")) then
	...
	if(e.message:findi("mystical sewing kit")) then
	...
	if(e.message:findi("key ingredients")) then
	...
	if(e.message:findi("understand")) then
		if(((" .. e.other:Class() .. " == 'wizard') or(" .. e.other:Class() .. " == 'magician') or (" .. e.other:Class() .. " == 'enchanter')) and (" .. e.other:Race() .. " == 'dark elf')) then
			e.self:Say("'Very well " .. e.other:GetName() .. ", here is your Mystical sewing kit. I have the patterns and recipes necessary to craft Spurned Initiate [Skullcaps], [Bracers], [Sandals], [Sleeves], [Trousers], [Glovesend and [Robes]. When you feel you are ready to gather the items necessary to craft your armor simply tell me which piece you want to craft. The items that you will need to collect will mostly be found in the Forest but some will be here in town. As I said before seek out the Arcane Lightwalkers to collect the cloaks that you will need to make the armor materials. I look forward to seeing what you will make of this opportunity.");
	...
	if(e.message:findi("skullcaps")) then
	...
	if(e.message:findi("trousers")) then
	...
	if(e.message:findi("final test")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 22642, item2 = 22643, item3 = 22644})) then
		...
		e.other:SummonItem(22647); --Ceremonial Dagger of the Spurned
	end
