...
function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say("Greetings and Salutations " .. e.other:GetName() .. "! I am Grimel, aid to the mighty paladin Councilman Taldarius. In between his duties to the city, Taldarius and I spend most of our time in the Planes cleansing evil. Even now I am preparing for battle. Taldarius intends to go further into the Planes than ever before on our next trip and I am working on gathering the special supplies we need. If you are willing to help I may have some use for your skills if you are a [" .. eq.say_link("master of trades") .. "] and have traveled in the Planes.");
	
	--Aid Grimel Earring #1:  Must have killed Bertox and rescued Mylik and Adler to begin quest
	elseif(e.message:findi("master of trades") and (qglobals.pop_cod_final) ~= nil) then 
		e.self:Say("Excellent! I am looking for a smith to add a special imbue to Councilman Taldarius's armor. Are you well [" .. eq.say_link("versed in the art of smithing") .. "]?");
	elseif(e.message:findi("versed in the art of smithing") and qglobals.pop_cod_final ~= nil) then
	...
	--Aid Grimel Earring #2:  Must have completed Terris Thule and received final flag from Thelin
	elseif(e.message:findi("brewing skill") and e.other:HasItem(16249) and (qglobals.pop_ponb_poxbourne) ~= nil) then --Requires Hardened Leather Signet 
	...
	--Aid Grimel Earring #3:  Must have completed Saryrn questline
	elseif (e.message:findi("jewel craft skills to the test") and e.other:HasItem(16250) and qglobals.pop_pot_saryrn_final ~= nil) then --Requires Clay Signet
	...
	--Aid Grimel Earring #4: Must have spoken with Maelin after killing VZ/TZ to go after Rallos Zek
	elseif (e.message:findi("ready to use some clay") and e.other:HasItem(16251) and qglobals.pop_maelin_info_zek_bros ~= nil) then --Requires Wooden Signet
	...
	--Aid Grimel Earring #5: Must have full elemental access plus PoFire
	elseif (e.message:findi("skilled with the needle") and e.other:HasItem(16252) and qglobals.pop_elemental_grand_librarian ~= nil and qglobals.pop_sol_ro_solusk ~= nil) then --Requires Metal Signet
	...
	--Aid Grimel Earring #6: Must have full elemental access plus PoFire
	elseif (e.message:findi("fletching knife") and e.other:HasItem(1079) and qglobals.pop_elemental_grand_librarian ~= nil and qglobals.pop_sol_ro_solusk ~= nil) then --Requires Marked Signet
	...
	--Aid Grimel Earring #7: Must have full elemental access plus PoFire
	elseif (e.message:findi("master chef") and e.other:HasItem(16254) and qglobals.pop_elemental_grand_librarian ~= nil and qglobals.pop_sol_ro_solusk ~= nil) then --Requires Runed Signet
	...
	elseif (e.message:findi("master of trades") or e.message:findi("versed in the art of smithing") or e.message:findi("brewing skill") or e.message:findi("jewel craft skills to the test") or e.message:findi("ready to use some clay") or e.message:findi("skilled with the needle") or e.message:findi("fletching knife") or e.message:findi("master chef"))  then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 15985})) then --Imbued Breastplate
		e.self:Say("What a wonderful job! Councilman Taldarius shall wear this on our next adventure, I am sure he will find it more protective than his old one. Take this signet as a token of my gratitude. If you are not too busy I have another task, do you have any [" .. eq.say_link("brewing skill") .. "]?");
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 15993, item2 = 17179})) then --Portable Drink and Portable Drink Barrel
		e.self:Emote("gulps down a Twice Brewed Constitutional and burps loudly! 'Ahhhh that was refreshing! That should hold me over for quite some time. I see you are quite deft of hand, perhaps you would care to put your [" .. eq.say_link("jewel craft skills to the test") .. "]?'");
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 15991})) then --Velium Blue Diamond Ring
		e.self:Emote("grins. 'Quite a nice ring you have made for me " .. e.other:GetName() .. ". May it serve me as well as my old ring. Here take this signet. We seem to be finishing the tasks on my list at a nice pace. Tell me when you are [" .. eq.say_link("ready to use some clay") .. "].");
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 16246})) then --Filled Sacred Urn
		e.self:Say("Outstanding work " .. e.other:GetName() .. "! I can feel the purity of the water radiating through the clay. Are ye [" .. eq.say_link("skilled with the needle") .. "] as well as an accomplished potter?");
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 15986})) then --Fire Undergarment Tunic
		e.self:Say("Good work " .. e.other:GetName() .. ". This will definitely serve Councilman Taldarius well. Here take this! If you have skills with a [" .. eq.say_link("fletching knife") .. "] I may have a job for you to do.");
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 16247})) then --Signet Featherwood Bow
		e.self:Say("Masterful work " .. e.other:GetName() .. "! I can see your skill in the curves of the bow. Take this as a sign of my respect for your skill. The last item I need before we can be off is some food. I warn you this will truly test your skills. Do you fancy yourself a [" .. eq.say_link("master chef") .. "]?");
