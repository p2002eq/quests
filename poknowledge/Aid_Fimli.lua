...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings " .. e.other:GetName() .. "! I wish I had time to chatter but my duties to the list and the Grand Librarian keep me extremely busy.");
	elseif(e.message:findi("duties")) then
		e.self:Say("I am the aid for the Grand Librarian and he has me cataloging tomes on the master list. I spend so much time working on the list I haven't had time to go collect these items from the planes my uncle in Ak'Anon keeps pestering me over. He wants to upgrade his scrapyard back in Ak'Anon, and says only these items will do. Would you be willing to do me a favor?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 15980})) then --Note to Fimli
		...
		e.other:QuestReward(e.self,0,0,0,0,15807); --Cloak of Lesser Pernicity
	end
