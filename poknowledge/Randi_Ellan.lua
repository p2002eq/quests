...
function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Say("And a good morning to you as well, " .. e.other:GetName() .. ". I can see by the gleam in your eye you are interested in learning more about the flowers of Norrath. Sadly my exhibit is not much to speak of. I require new [" .. eq.say_link("specimens") .. "] to help build my collection.");
	elseif(e.message:findi("specimens")) then
		e.self:Say("If you could find some for me, I would be ever so grateful. I need a Black Root, Maneater Bud, Yew Leaf, and a Twilight Orchid. If you should come across them, handle them with care. Be sure to get a Collector's Box from [" .. eq.say_link("Merri") .. "] to keep them in. Close the box and return it to me.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 28083})) then -- Collection of Plants
		...
		e.other:QuestReward(e.self,0,0,0,0,28238,100); -- Fine Antique Corset
	end
