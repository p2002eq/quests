...
function event_say(e)
	if(e.message:findi("Hail") and e.other:HasItem(1057)) then
		e.self:Say("Wow, you have a Mystic Cloak! I was just reading a book about cloaks with similar enchantments. I'm studying to try and learn how to make magical and enchanted armor. I'm pretty sure I have figured out a way to [" .. eq.say_link("recharge") .. "] a Mystic Cloak.");
	elseif(e.message:findi("Hail")) then
		e.self:Say("Hello there.  Fine afternoon. isn't it?");
	elseif(e.message:findi("recharge")) then
		e.self:Say("Well, in the case of the Mystic Cloak at least, the true enchantment is in the brooch that you use to clip the cloak around your shoulders. The cloak itself cannot run out of magic, the brooch, however, can. When it no longer has the ability to cast spells you should be able to attach a [" .. eq.say_link("new brooch") .. "] and regain the magic the cloak once possessed.");
