...
function event_say(e)
	...
	if(qglobals["AidEinoDone"] ~= nil and (qglobals["AidEinoDone"] == 1)) then --Can't do the turn in without getting the bowl, so I'm not worried about restricting the turn ins on the flag, just this text and getting the bowl.
		...
		if(e.message:findi("hail")) then
			e.self:Say("Greetings " .. e.other:GetName() .. ". I am Councilwoman Kerasha, representative of the Selia district. I provide the voice of nature to the Council of Tanaan. Woe is the city in any realm that forgets to heed Mother Tunare. For where else could one draw strength from but the land?");
		end
	end
end
