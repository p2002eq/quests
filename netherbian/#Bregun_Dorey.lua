...
function event_say(e)
	if e.message:findi("hail") then
		e.self:Say("Trog trog trog [" .. eq.say_link("trog") .. "]?");
	elseif e.message:findi("trog") then
		e.self:Say("Ha! What are you talking about?! Strange thing you are. Wait a minute, come closer. Are you from [" .. eq.say_link("Norrath") .. "]?");
	...
	elseif e.message:findi("norrath") then
		e.self:Say("I thought so, your armor looks to be made from things found not here. Have you heard of [" .. eq.say_link("Rivervale") .. "]?");
	elseif e.message:findi("Rivervale") then
		e.self:Say("[" .. eq.say_link("Good") .. "].");
	elseif e.message:findi("good") then
		e.self:Say("Yeah good! Trog trog trog trog! Hahaha! These Troglodytes don't know of Rivervale. They just run around the cave all day doing Troglodyte things. Hahaha! Trog trog trog. So what was I speaking of.. Oh Bixies! So, are there [" .. eq.say_link("bixies") .. "] around Rivervale?");
	end
end
...
function event_trade(e)
	...
	if (item_lib.check_turn_in(e.self, e.trade, { item1 = 13464, item2 = 13464, item3 = 13464, item4 = 13464 })) then -- 4 bixie crunchies
		e.self:Say("Ah hum!");
