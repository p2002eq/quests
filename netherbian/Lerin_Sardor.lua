...
function event_say(e)
    if progress == 0 then
        if e.message:findi("hail") then
            e.self:Say("Ahh, hello there " .. e.other:GetName() .. ". I am a trader from Sanctus Seru trying to make my way to Shadowhaven for some dealings. I have come this far with the aid of guards from Sanctus Seru, but they have turned back and left me to go on my own. I hear there are many terible [" .. eq.say_link("creatures") .. "] here lurking in the shadows, leave it to the guards of the Hand to leave me unprotected.'");
        elseif e.message:findi("creatures") then
            e.self:Say("What I was told is that this pass is filled with small horrible creatures with no eyes. Perhaps you could [" .. eq.say_link("help by providing safe passage") .. "] to the other side?");
...
function spawn_wave(e_self)
    ...
    for i=1,math.random(2,4) do
        eq.spawn2(mob, 0, 0, my_x+math.random(-20, 20), my_y+math.random(20,80.0, my_z, 0):AddToHateList(e_self);
    end
end
