function event_say(e)
	...
	if(e.message:findi("hail")) then
		e.self:Say("Step forth, young " .. e.other:GetName() .. "! I heard that you have come of age! The spirit of adventure has entered your body. That is good. Go and speak with the others. They shall help you. I am afraid I have no time to spend conversing. There is much I have to [" .. eq.say_link("ponder") .. "].");
	elseif(e.message:findi("ponder")) then
		if(e.other:GetFaction(e.self) < 5) then
			e.self:Say("Must you know everyone's business? Hmm.. Maybe you can be of assistance. You see, I have been instructed by Furtog to tend to a matter of extreme urgency, which is keeping me from clearing the mines of rats. Will you assist and [" .. eq.say_link("exterminate the rats") .. "]?");
	...
	elseif(e.message:findi("exterminate")) then
		if(e.other:GetFaction(e.self) < 5) then
			e.self:Say("Very good! I shall reward you for every four giant rat pelts returned to me. And be on the lookout for a [" .. eq.say_link("metal rat") .. "]!");
		elseif(e.other:GetFaction(e.self) == 5) then
			e.self:Say(prove);
		else
			e.self:Say(shifty);
	...
	elseif(e.message:findi("eye")) then
		if(e.other:GetFaction(e.self) < 6) then
			e.self:Say("The great statue of Kazon Stormhammer was once encrusted with two great gems from the mines of Butcherblock. So big were they that it took the magic of the high elves to assist us in lifting them to the statue's face. In the year 2995, somehow, someone stole one of the eyes. We decided to keep the remaining eye in the vault. Now, even that has been stolen from us! Only the most trusted warriors may be involved in this [" .. eq.say_link("important Stormguard matter") .. "].");
		else
			e.self:Say(shifty);
		end
...
function event_trade(e)
	...
	if((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 13054, item2 = 13054, item3 = 13054, item4 = 13054}))) then -- Giant Rat Pelt
		e.self:Say("Great work, young one! We shall soon rid our mines of these pests. Keep a lookout for that [" .. eq.say_link("metal rat") .. "]. Here is a small reward for such fine work. Soon, you shall be know as Kaladim's resident exterminator.");
		e.other:Ding();
	...
	elseif((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 13282}))) then -- Scrap Metal
		e.self:Say("I thank you, my friend. I was to destroy this metal monster months ago. I could never find him. Please accept this reward for such good service. Oh yes.. And take this card to a man named Doran Vargnus. He is a fine blacksmith. I am sure he will reward you with one of his finest suits of armor. Perhaps you may now assist in an [" .. eq.say_link("important Stormguard matter") .. "].");
