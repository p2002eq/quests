...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Emote("brushes pollen off of his tunic. 'Aachoo!! Oh!! Hello. Please look around. Unless you are a [" .. eq.say_link("druid looking for work") .. "].'");
		e.self:DoAnim(55);
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12960, item2 = 12955, item3 = 12958})) then
		e.self:Emote("tosses the mantrap root out the window.. SPLASH!! 'I made a mistake. I didn't need that one. Here is the shillelagh I told you about. I found it in some burned out woods far from here. I cleaned it up and found it had a spark of mana so I had it enchanted with a few charges of my spell, [" .. eq.say_link("Fertile Crop") .. "]. Hope you like it.'");
		e.other:Faction(418,10); --inhabitants of firiona
		e.other:Faction(92,7); --emerald warriors
		e.other:Faction(314,7); --storm guard
