function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Emote("looks a bit [" .. eq.say_link("What is wrong",false,"troubled") .. "]. 'Welcome, Welcome adventurer, I am the wizard tutor Despondo, although few need tutorage around here. It would seem that this is no place for a student of any sorts. The lands of Kunark are quite deadly.'");
		e.self:DoAnim(28);
	elseif(e.message:findi("wrong")) then
		e.self:Say("I was bound to this post by General Veredeth. I came to find the lost souls of my friends, but am needed here. I must find a fellow wizard to [" .. eq.say_link("rescue their souls") .. "].");
	elseif(e.message:findi("rescue their souls")) then
		e.self:Say("You will need the [" .. eq.say_link("three bottles of entrapment") .. "]. If you can find their remains in Kunark then you will give the bottle to the body and the soul shall enter. Return all three bottles and I shall give you the wizard spell [" .. eq.say_link("Brain Bite") .. "].");
	...
	elseif(e.message:findi("brain bite")) then
		e.self:Say("Brain Bite is a concussion spell created by my former friends whose souls are now lost to Kunark. The blow to the inner brain of an enraged foe may just cause the loss of memory and lower their rage. If you want a copy you have to [" .. eq.say_link("where are their souls",false,"rescue their souls") .. "].");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 10021})) then
		e.other:QuestReward(e.self,7,12,3,0,12964); -- Ryla's bottle
		e.self:Say("Identify this when the time comes. It is Ryla's prison.");
	-- Peridot Hand in
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 10028})) then
	...
	-- Souls hand in
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 12967, item2 = 12966, item3 = 12965})) then
