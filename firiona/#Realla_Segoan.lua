function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, " .. e.other:GetName() .. ". I am Realla Segoan from the city of Erudin. I cannot spend time chatting with you now. I have [" .. eq.say_link("official tasks") .. "] to perform.");
	elseif(e.message:findi("official tasks")) then
		e.self:Say("I have been sent to this continent by the High Council of Erudin to find information about the disappearance of [" .. eq.say_link("Al'Kabor") .. "].");
	elseif(e.message:findi("al'kabor")) then
		e.self:Say("Al'Kabor is believed to be the most powerful living wizard on Norrath. In addition, he is also one of Erudin's oldest citizens. The High Council is very interested in receiving information about his disappearance. So far, from the [" .. eq.say_link("reports") .. "] that I have collected, there appears to have been a [" .. eq.say_link("troublemaking gnome") .. "] involved.");
	elseif(e.message:findi("reports")) then
		e.self:Say("I have received many reports regarding the disappearance. Adventurers upon this continent have reported sightings of some of Al'Kabor's [" .. eq.say_link("missing research notes") .. "].");
	...
	elseif(e.message:findi("missing research notes")) then
		e.self:Say("It appears that through his travels, the great wizard has misplaced some of his research notes. Perhaps if I can find someone to [" .. eq.say_link("help collect the research notes") .. "], the High Council may be able to determine what caused Al'Kabor's disappearance. However, only those who truly know the path the great wizard's research has taken him will know where to find the missing research notes.");
