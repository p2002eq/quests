...
local item_lib = require('items');
	if(e.other:GetFaction(e.self <= 3)) then --Warmly or better
		if (item_lib.check_turn_in(e.self, e.trade, {item1 = 25119})) then
			e.self:Say("The bards will sing of your greatness, $name. Rage on like the Maelstrom when you wear this cloak. You have done a great service for the city of Kael Drakkel, and we Kromzek do not soon forget great deeds.");
			e.other:SummonItem(25023);
		elseif(item_lib.check_turn_in(e.self, e.trade, {item1 =  25118})) then
			e.self:Say("We shall hang this head from the halls of Dragondeath Keep and sing your praises, $name. Take my axe as a reward for your great deeds. You are a true hero for Kael Drakkel!");
