function event_say(e)
	if (e.message:findi("hail")) then
		e.self:Say("Greetings, ".. e.other:GetName() .. ", I have heard of you. My name is Wenglawks Kkeak. Welcome to my humble home. My sources tell me you are a [" .. eq.say_link("mercenary") .. "]. Is this true?");
	elseif (e.message:findi("mercenary")) then	
		e.self:Say("Well then, I suppose you might be [" .. eq.say_link("looking") .. "] for work. Are you?");
	elseif (e.message:findi("looking")) then	
		e.self:Say("I have been looking for a few good people to deliver goods for me. I have several [" .. eq.say_link("deliveries") .. "] to clients which must be made soon. My last delivery person ended up [" .. eq.say_link("dead") .. "], unfortunately.");
	elseif (e.message:findi("deliveries")) then	
		e.self:Say("I have three clients whose orders must be taken care of. These will not be simple deliveries. I want you not only to deliver the items but to assist them if they need help using their packages. Do you wish to hear of the [" .. eq.say_link("first task") .. "] I have for you?");
	elseif (e.message:findi("first task")) then	
		e.self:Say("One of my clients lives on the shores of the Iceclad Ocean. His name is Sojan. Supposedly he is some kind of pirate from the other lands. However, I could not care less what he calls himself. He paid me well to acquire him a set of Coldain tools and they were not easy to come by. Are you willing to take this [" .. eq.say_link("package") .. "] to Sonjan?");
	...
	elseif (e.message:findi("third client")) then	
		e.self:Say("Are you sure you will be up to this journey? It will take a hardened person to survive the trip. Let alone find my client. Do you really [" .. eq.say_link("want to go") .. "] out into the dangerous lands in the west?");
	elseif (e.message:findi("want to go")) then	
		e.self:Say("I'm not sure if I can [" .. eq.say_link("trust") .. "] you with this. If you die on the trip, I will be losing profit! The other deliveries were much simpler, I remind you. Can I really [" .. eq.say_link("trust") .. "] you as far as I can throw you? Wait, that's a bad example - I could probably throw you quite far.");
...
function event_trade(e)
	...
	elseif (item_lib.check_turn_in(e.self, e.trade, {item1 =  20474})) then	-- Receipt
		e.self:Say("I assume the delivery went well? You can have these as payment. One of my customers just decided he does not wish to buy them. They are rather sturdy! I do have another task available if you wish to [" .. eq.say_link("undertake") .. "] it.");
	...
	elseif (item_lib.check_turn_in(e.self, e.trade, {item1 =  25130})) then	-- Giant Icewurm Talisman
		e.self:Say("He has reconsidered the offer, you say? This is most excellent! I hope this will be acceptable payment for your troubles, $name. My cash situation is not the best at this point in time. I do have a [" .. eq.say_link("third client") .. "] who needs delivery...");
