function event_trade(e)
	...
	if (item_lib.check_turn_in(e.self, e.trade, {item1 = 25279})) then
		e.self:Say("Here you go $name. I dunno why master Wenglawks trusts you with this but take it anyways and don't mess up.");
		e.self:Emote("smacks his fists together as he looks at you.");
		e.other:SummonItem(25104);
	elseif (item_lib.check_turn_in(e.self, e.trade, {item1 =  25280})) then		
		e.self:Say("Another delivery eh? Be careful with this spear, if you break it I'll break you.");
		e.other:SummonItem(25106);
	elseif (item_lib.check_turn_in(e.self, e.trade, {item1 =  25281})) then	
		e.self:Say("It's hard to belive a $race like you is going to make a trip like this. I don't envy you little fool.");
		e.other:SummonItem(25107);
