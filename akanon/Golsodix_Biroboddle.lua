...
function event_say(e)
    if(e.message:findi("Hail")) then
        e.self:Say("Hello there Gnormy. I must admit that I am not in the highest of spirits on this day. So unless you know something about [" .. eq.say_link("Who is Xoomix?", false, "Xoomix") .. "], please leave me alone.");
    elseif(e.message:findi("find")) then -- "find" block needs to be above "Xoomix" block!
    ...
    elseif(e.message:findi("overtook") or e.message:findi("overtaken")) then
        ...
        e.other:SummonItem(321979) -- Clockwork Eye
    end
end
...
function event_trade(e)
    ...
    if(item_lib.check_turn_in(e.self, e.trade, {item1 = 321984})) then -- Tinkered Contraption
        ...
        e.other:SummonItem(321985); -- Xoomix's Gearbox
