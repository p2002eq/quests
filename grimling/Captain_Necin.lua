...
function event_trade(e)
	...
	elseif (officer and item_lib.check_turn_in(e.self, e.trade, {item1 = 4399})) then 	-- Grimling Officer's Toes
		...
		eq.spawn2(167200, 0, 0, -1109, -598, 9,260.0);	--repop captain
		eq.depop();
	end
...
function event_waypoint_arrive(e)
	...
	elseif e.wp == 5 then
		if eq.get_entity_list():IsMobSpawnedByNpcTypeID(sergeant) then
			eq.get_entity_list():GetMobByNpcTypeID(sergeant):GMMove(865,-462,85,376.0,true);
		else
			eq.spawn2(sergeant,0,0,865,-462,85,376.0);
		end
	elseif e.wp == 7 then
