	...
	if (e.other:GetFaction(e.self) == 1) then  	--check for ally faction to receive dialogue
		...
		if (qglobals["garzicor"] == "6") then
			...
			elseif((e.message:findi("yes") and e.message:findi("master smith")) or e.message:findi("I am a master smith")) then
				e.self:Say("Good. The weapon is extremely complicated to construct, your skills will be tested perhaps beyond your abilities. The weapon is known as a Naginata. You need not be proficient in its use to build it. The Naginata is constructed of four major parts. [" .. eq.say_link("The shaft") .. "], [the blade], [the fittings], and [the hilt], or tsuba. No ordinary materials can hope to bind the dragons spirit. Special materials must be found.");
			...
			elseif(e.message:findi("the hilt")) then
				e.self:Say("The Tsuba is where most of the enchantment will lie. It must be forged seperately of another ore created to be the bane of the dragon. It is called [" .. eq.say_link("chronium") .. "] and was developed by the Kromzek with the aid of their blood-thirsty @#%$. You must forge the ore into a plate and then find a [craftsman] with the skill to carve a detailed mandala of draconic binding into it. This mandala will hold arcane power and restrain the spirit from destroying the weapon.");
			...
			elseif(e.message:findi("craftsman")) then
				e.self:Say("Since the [" .. eq.say_link("mandala") .. "] is so detailed, only a craftsman trained specifically in these matters will do. I'm sure you're a fine smith but an adventurer like yourself wouldn't have time to fully master the skill needed to create the tsuba. Find one whose entire life is the art of creating these types of weapons. The craftsman will need a sketch of the mandala as well as the plate of chronium.");
			elseif(e.message:findi("mandala")) then
				e.self:Say("The type of mandala we need is a diagram laced with powerful arcane enchantment. You'll need to find a sketch of the correct mandala for the craftsman to go by as he carves it into the chronium plate. There is one [" .. eq.say_link("place") .. "] you may be able to find a sketch of a mandala powerful enough to bind Garzicor.");
