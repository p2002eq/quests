...
function event_say(e)
	...
	if (e.other:GetFaction(e.self) == 1) then  	--check for ally faction
		...
		elseif(e.message:findi("thank you")) then
			...
			e.self:Emote("'s eyes suddenly begin to shine with a magical, golden, light. He looks down upon you as if in judgement and asks, 'Tell me, " .. e.other:GetName() .. ", have you done [" .. eq.say_link("battle with dragons") .. "]?'");
		elseif((e.message:findi("yes") and e.message:findi("battle with dragon")) or (e.message:findi("i have done battle with dragon"))) then
			e.self:Emote("'s eyes slowly begin to lose the golden light that flared earlier. He then studies you carefully before saying 'There are many Dragons on this world that do not share our beliefs. If you have done battle with dragons and have lived, then you may be able to [" .. eq.say_link("help") .. "] us."); 
