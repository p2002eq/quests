function event_say(e)
  if (e.other:GetFaction(e.self) <= 2) then --warmly or lower
    if (e.message:findi("hail")) then
      e.self:Say("Welcome to Skyshrine. I have noticed a great deal more of your people joining the ranks of the shrine, hoping to assist in pushing back the giants for good. I am here as the commander of the militia set forth by Lord Yelinak himself. With that in mind, it is my duty to assign certain [" .. eq.say_link("tasks") .. "] to those who are willing to show their support to the kin, and to the shrine.");
    elseif(e.message:findi("task")) then
        e.self:Say("Well, for instance, we have recently acquired information from one of our scouts in the wastelands to the west that there have been storm giants wandering the coastal area nearby. We have not heard from this scout in some time however, and we are beginning to worry about their status. Your task, should you choose to [" .. eq.say_link("accept") .. "] it, is to deliver some tools to the scout so that they can communicate back to us with their status, and the status of those giants.");
