function event_say(e)
	...
	elseif(e.message:findi("draconic drum")) then
		e.self:Say("The frame of the drum must be crafted from Kromrif Bones and the skin of the drum must be crafted from a [Cured Brontotherium Hide]. If you bring me those items I will craft the drum for you and have it inscribed with the dragon script runes.");
	elseif(e.message:findi("draconic lute")) then
		e.self:Say("The strings of the draconic lute must be crafted from Siren Hair and the body of the lute must be carved from a Brontotherium Femur treated with a [special shellac]. If you bring me those items I will craft the lute for you and have it inscribed with the dragon script runes.");
	elseif(e.message:findi("cured brontotherium hide")) then
		e.self:Say("First you must hunt down a brontotherium and skin its hide. Once you have the hide it needs to be cured with a [special tannin] in a tailoring kit.");
	elseif(e.message:findi("special tannin")) then
		e.self:Say("The tannin used to cure the brontotherium hide must be created by a brewer from a Flask of Water, an [Oily Goo Secretion], and one [Melted Glacier Whiskey].");
