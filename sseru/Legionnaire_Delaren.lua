...
function event_say(e)
    if(e.message:findi("hail")) then
        e.self:Say("Well met " .. e.other:GetName() .. ". are you the new recruit that was sent to me to help exterminate the [" .. eq.say_link("Zelniaks") .. "] that attack our guards and caravans?");
    elseif(e.message:findi("zelniaks")) then
        e.self:Say("Weren't you briefed on this already? The zelniaks are highly aggresive scaled creatures with a large mane of fur that runs down the length of their back. They are a constant problem for the guards and caravans that used to travel in [" .. eq.say_link("dawnshroud") .. "].");
    elseif(e.message:findi("dawnshroud")) then
        e.self:Say("are you new here or something? The Dawnshroud Peaks are a range of mountains lush in vegetation and water. We used to utilize the area as a trade route to Shadow Haven. The caravans that would travel through were often attacked by roaming zelniaks. I thought you were the one that was sent to me to [" .. eq.say_link("help lower") .. "] the amount of zelniaks in the area. So the trade route could be reestablished.");
...
function event_trade(e)
    ...
    if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10692, item2 = 10692, item3 = 10692, item4 = 10692})) then -- 4x Zelniak Meat
        e.self:Say("Although your service to Seru is appreciated, I just recieved word that you were not the one that was sent to do this menial task. That lazy boy [" .. eq.say_link("Peran") .. "] was supposed to take the job.");
