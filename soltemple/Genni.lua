...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("I am Genni of the temple of Solusek Ro. I serve as custodian of the [" .. eq.say_link("lambent stones") .. "].");
	elseif(e.message:findi("lambent stones")) then
		e.self:Say("Lambent stones are receptacles of power. Alone they are not useful, but when combined with other gemstones they can be used for making magic items. I know of [" .. eq.say_link("star ruby") .. "] lambent, [ruby] lambent, [sapphire] lambent and [fire opal] lambent.");
	...
	elseif(e.message:findi("fire opal")) then
		e.self:Say("My Brother Ostorm holds the secret to making fire opal lambent stones. Though he will not speak of it, if you give him two fire opals and a lambent stone, he will make you fire opal lambent. If you are interested, I can [" .. eq.say_link("sell") .. "] you a [" .. eq.say_link("fire opal") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10032, item2 = 10032, item3 = 10000})) then
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {gold = 550})) then
		e.other:Faction(320,5);
		e.other:Faction(291,-5);
