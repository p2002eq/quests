function event_say(e)
	if(e.message:findi("Hail")) then
		e.self:Say("How's it going. scrub? Correct me if I'm wrong. but you appear to be a rogue. If you're a [" .. eq.say_link("I am a new rogue",false,"new rogue") .. "]. it's my job to test your skill.  If you are not. then by speaking with me. I see you show interest in [" .. eq.say_link("I am joining this house of rogues",false,"joining this house of rogues") .. "].");
	elseif(e.message:findi("i am a new rogue")) then
		e.self:Say("We of the Ebon Mask must first be sure you are skilled with a blade. At the same time. we are attempting to create a new type of armor for our rogues. So it is the test of each young rogue to take this bag into Nektulos and fill it with three moss snake skins and three snake fangs. then combine them. We get the bag and you earn a little respect as well as the [" .. eq.say_link("Dark Shield of Ebon") .. "].");
		e.other:SummonItem(17943);
	elseif(e.message:findi("dark shield of ebon")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13889})) then
		e.self:Say("Fine work for a scrub. Maybe one day you will be able to test this new snakescale armor out. For the moment you shall take the [" .. eq.say_link("Dark Shield of Ebon") .. "] if you do not own one already.");
