...
function event_say(e)
    ...
    elseif(e.message:findi('brave')) then
        ...
        local zoneGlobal = "POWATER-" .. tostring(e.other:GuildID());
        ...
        if (instanceId ~= nil) then
            ...
            e.other:MovePCInstance(216, tonumber(instanceId), -165,-1250,10,115);
        else
...
local static_named = {};	--disable these spawnpoints since they don't share other trash NPCS (also are included above in disabled instance)

function event_spawn(e)
    local instance_id = eq.get_zone_instance_id();
    if(instance_id ~= 0) then
		local mysql = require("luasql_ext");
		for id,name in mysql.rows(con,"SELECT id from npc_types WHERE id >= 216000 and id < 217000 and disable_instance = 1") do
