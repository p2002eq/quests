...
function event_spawn(e)
	...
	elseif (qglobals[instance_id.."_potimeb_status"] == "Phase1") then
		...
		eq.set_global(instance_id.."_potimeb_phase_bit","0",7,"H13");
		-- Spawn phase 1
		eq.spawn2(223169,0.0,0,13.5,1632.4,492.3,0); -- earth trigger
		eq.spawn2(223170.0,0,0,10.1,1350,492.6,0); -- air trigger
		eq.spawn2(223171,0.0,0,18.0,1107,492.2,0); -- undead trigger
		eq.spawn2(223172,0.0,0,11.5,857,492.5,0); -- water trigger
		eq.spawn2(223173,0.0,0,13.2,574.2,492.3,0); -- fire trigger
	...
	elseif (qglobals[instance_id.."_potimeb_status"] == "Phase6") then
		...
		eq.spawn2(223228,0.0,0,-579,-1119,60.625,0);
	end
end
...
function event_signal(e)
	...
	elseif (e.signal == 8) then
		...
		for c in client_list.entries do
			if (c.valid) then
				c:MovePC(202,1015,20,392,264.0);
			end
		end
...
function SpawnPhaseTwo()
	if (eq.get_spawn_condition("potimeb",instance_id,10) == 0) then
		...
		-- Earthen_Overseer
		eq.spawn2(223134,0,0,262,1644,493,385.0);
		-- Windshapen_Warlord_of_Air
		eq.spawn2(223118,0,0,262,1354,493,385.0);
		-- Ralthos_Enrok
		eq.spawn2(223127,0,0,262,1109,493,385.0);
		-- War_Shapen_Emissary
		eq.spawn2(223096,0,0,262,869,493,385.0);
		-- Gutripping_War_Beast
		eq.spawn2(223146,0,0,262,574,493,385.0);
	end
end
...
function ControlPhaseThree()
	if (current_phase == "Phase3") then
		...
		eq.spawn2(223010,0,0,1280,1010,359.38,390.0); -- A_Needletusk_Warboar
		eq.spawn2(223011,0,0,1280,1030,359.38,390.0); -- Deathbringer_Rianit
		eq.spawn2(223012,0,0,1260,1250,359.38,390.0); -- Sinrunal_Gorgedreal
		eq.spawn2(223013,0,0,1260,1270,359.38,390.0); -- Herlsoakian
		eq.spawn2(223014,0,0,1280,1210,359.38,390.0); -- Xerskel_Gerodnsal
		eq.spawn2(223015,0,0,1280,1190,359.38,390.0); -- Dersool_Fal`Giersnaol
		eq.spawn2(223016,0,0,1260,970,359.38,390.0); -- Xeroan_Xi`Geruonask
		eq.spawn2(223017,0,0,1260,950,359.38,390.0); -- Kraksmaal_Fir`Dethsin
		eq.spawn2(223018,0,0,1300,1070,359.38,390.0); -- Dreamwarp
		eq.spawn2(223019,0,0,1300,1090,359.38,390.0); -- Champion_of_Torment
		eq.spawn2(223020,0,0,1300,1130,359.38,390.0); -- Dark_Knight_of_Terris
		eq.spawn2(223021,0,0,1300,1150,359.38,390.0); -- Undead_Squad_Leader
		eq.spawn2(223022,0,0,1230,1330,359.38,350.0); -- A_Deadly_Warboar
		eq.spawn2(223023,0,0,1230,1310,359.38,350.0); -- Deathbringer_Skullsmash
		--eq.spawn2(223155,0,0,1250,1135,359.5,384.0); -- A_Ferocious_Warboar
		--eq.spawn2(223156,0,0,1250,1085,359.5,384.0); -- Deathbringer_Blackheart
		-- depop untargetable and pop targetable versions
		--eq.depop(223155); -- A_Ferocious_Warboar
		--eq.depop(223156); -- Deathbringer_Blackheart
		eq.spawn2(223008,0,0,1250,1135,359.5,384.0); -- A_Ferocious_Warboar
		eq.spawn2(223009,0,0,1250,1085,359.5,384.0); -- Deathbringer_Blackheart
	...
	elseif (current_phase == "Phase3.1") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223024,0,0,1250,1085,359.5,384.0); -- Kraksmaal_Fir`Dethsin
			eq.spawn2(223025,0,0,1250,1135,359.5,384.0); -- Xeroan_Xi`Geruonask
		end
	elseif (current_phase == "Phase3.2") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223031,0,0,1250,1085,359.5,384.0); -- Deathbringer_Skullsmash
			eq.spawn2(223032,0,0,1250,1135,359.5,384.0); -- A_Deadly_Warboar
		end
	elseif (current_phase == "Phase3.3") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223038,0,0,1250,1085,359.5,384.0); -- Sinrunal_Gorgedreal
			eq.spawn2(223037,0,0,1250,1135,359.5,384.0); -- Herlsoakian
			end
	elseif (current_phase == "Phase3.4") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223047,0,0,1250,1085,359.5,384.0); -- A_Needletusk_Warboar
			eq.spawn2(223046,0,0,1250,1135,359.5,384.0); -- Deathbringer_Rianit
		end
	elseif (current_phase == "Phase3.5") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223050,0,0,1250,1085,359.5,384.0); -- Dersool_Fal`Giersnaol
			eq.spawn2(223051,0,0,1250,1135,359.5,384.0); -- Xerskel_Gerodnsal
		end
	elseif (current_phase == "Phase3.6") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223058,0,0,1250,1085,359.5,384.0); -- Dark_Knight_of_Terris
			eq.spawn2(223057,0,0,1250,1135,359.5,384.0); -- Undead_Squad_Leader
		end
	elseif (current_phase == "Phase3.7") then
		...
		if (event_counter == 10) then
			...
			eq.spawn2(223066,0,0,1250,1085,359.5,384.0); -- Dreamwarp
			eq.spawn2(223065,0,0,1250,1135,359.5,384.0); -- Champion_of_Torment
		end
	elseif (current_phase == "Phase3.8") then
		...
		if (event_counter == 10) then
			...
			eq.clear_spawn_timers();
			-- spawn golems
			eq.spawn2(223073,0,0,1492,1110,374.1,391.0); -- Avatar_of_the_Elements
			eq.spawn2(223074,0,0,1563,1110,374.1,391.0); -- Supernatural_Guardian
		end
...
function SpawnPhaseFour()
	...
	if (phase_bit == 15) then
		...
		if (bit.band(phase_bit, 1) == 0) then
			eq.spawn2(223075,0,0,-310,307,365,190.0); -- Terris Thule
		else
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 2) == 0) then
			eq.spawn2(223076,0,0,-320,-316,358,65.0); -- Saryrn
		else
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 4) == 0) then
			eq.spawn2(223077,0,0,405,-84,358,384.0); -- Tallon Zek
		else
			event_counter = event_counter + 1;
		end
		if (bit.band(phase_bit, 8) == 0) then
			eq.spawn2(223078,0,0,405,75,358,384.0); -- Vallon Zek
		else
			event_counter = event_counter + 1;
		end
	end
...
function SpawnPhaseFive()
	...
	if (phase_bit == 15) then
		...
		if (bit.band(phase_bit, 16) == 0) then 
			if (bit.band(phase_bit, 1) == 0) then
				eq.spawn2(223098,0,0,-299,-297,23.3,62.0); -- Fake Bertoxxulous
			else
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 2) == 0) then
				eq.spawn2(223165,0,0,-257,255,6,203.0); -- Fake Cazic
			else
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 4) == 0) then
				eq.spawn2(223000,0,0,303.3,306,13.3,323.0); -- Fake Innoruuk
			else
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 8) == 0) then
				eq.spawn2(223001,0,0,264,-279,18.75,435.0); -- Fake Rallos
			else
				event_counter = event_counter + 1;
			end
			-- spawn the armies
		...
		-- Armies are dead, which gods should we pop
			...
			if (bit.band(phase_bit, 1) == 0) then
				eq.spawn2(2262.042,0,0,-299,-297,23.3,31); -- Real Bertoxxulous
			else 
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 2) == 0) then
				eq.spawn2(223166,0,0,-257,255,6,203.0); -- Real Cazic
			else 
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 4) == 0) then
				eq.spawn2(223167,0,0,303.3,306,13.3,323.0); -- Real Innoruuk
			else 
				event_counter = event_counter + 1;
			end
			if (bit.band(phase_bit, 8) == 0) then
				eq.spawn2(223168,0,0,264,-279,18.75,435.0); -- Real Rallos
			else 
				event_counter = event_counter + 1;
			end
		end
