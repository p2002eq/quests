function event_say(e)
	if(e.message:findi("boots of ro")) then
		if(e.other:GetFaction(e.self) < 5) then  -- amiable or better to get the quest
			...
			e.self:Say("You and I must be brothers and serve the Lord of Underfoot. Go to Kaladim and serve her cathedral. When you think you are ready. then ask Lord Datur if you are an [" .. eq.say_link("honored member") .. "] of the temple. If the answer is yes, then I will trust you.");
		end
	...
	elseif(e.message:findi("odd druid")) then
		e.self:Say("The mountains have been plagued not only by beasts, but by evil druids!! They attempt to force all men from this land. One has stolen the foot of the candlestick which holds the [" .. eq.say_link("Candle of Bravery") .. "]. I remember that battle cry of hers, 'Long live the green!!'.");
	end
end
...
function event_trade(e)
	...
	if((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 12206}))) then -- Kilij Plans
		e.self:Say("Ahh!!  The kilij!!  The legend was true.  As for you..  the mold for the Boots of Ro.  Go and speak with Thomas for the final component.  Ask him of [" .. eq.say_link("Lord Searfire") .. "].  Brell be with you.");
