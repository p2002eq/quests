function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("I, the keeper of the [" .. eq.say_link("Bracers of Ro") .. "],  welcome you.  Come and rest within our camp.  You have nothing to fear while such righteous might is gathered.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12311, item2 = 12311}) and e.other:GetFaction(e.self) < 5) then
		e.self:Say("You have done well. Take the mold for the bracer.  Go forth to speak with Thomas of [" .. eq.say_link("Lord Searfire") .. "].  Then all components shall be known.");
