...
function spawn_mob(NPCID, loc)
	...
	for n = 1,count do
		mobz = eq.spawn2(NPCID,0,0,spawn_loc[loc][1] + math.random(-10,20.0,spawn_loc[loc][2] + math.random(-10,10) ,spawn_loc[loc][3],spawn_loc[loc][4]);
	end	
...
function SpawnSummoners()  --154392 (cold) -- 154393 (magic) -- 154394 (fire) -- 154395 (poison)
	...
	eq.unique_spawn(summoner[roll][1],0,0,442,-297,37,390.0);	
	eq.unique_spawn(summoner[roll][2],0.0,0,433,-306,37,0);		
	eq.unique_spawn(summoner[roll][3],0,0,425,-297,37,130.0);		
	eq.unique_spawn(summoner[roll][4],0,0,433,-289,37,260.0);	
end
