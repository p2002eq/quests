...
function event_say(e)
	...
	elseif (e.message:findi("Greeting and good fortune to you, honored Dain Frostweaver IV")) then
		if (e.other:GetFaction(e.self) >= 5) then -- indifferent or worse
...
function event_trade(e)
	...
	if e.other:GetFaction(e.self) == 1 then
		if item_lib.check_turn_in(e.self, e.trade, {item1 = 30516}) then 
		...
		elseif item_lib.check_turn_in(e.self, e.trade, {item1 = 1465}) then
			...
			e.other:SummonItem(1567); --Declaration of War
			e.other:SummonItem(1465); --Dirk of the Dain (returned)
		end
	end
	
	-- indiff quest
	if e.other:GetFaction(e.self) <= 5 and  item_lib.check_turn_in(e.self, e.trade, {item1 = 1500, item2 = 30164}) then
	...
	-- no faction required
	if item_lib.check_turn_in(e.self, e.trade,{item1 = 1199}) or  item_lib.check_turn_in(e.self, e.trade, {item1 = 8895}) then -- either 7th shawl
