function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail, " .. e.other:GetName() .. ". I am the leader of this beleagured troop of rangers. Even though the once fair Kithicor slowly succumbs to the [" .. eq.say_link("darkness") .. "] at hand, we shall not abandon the woods without a fight.");
	elseif(e.message:findi("darkness")) then
		e.self:Say("The denizens of these woods are slowly being overrun by dark fiends and shadowed men. As valiantly as we have fought, our numbers dwindle. We know very little about our aggressors, but if we [" .. eq.say_link("acquire some information") .. "] or perhaps [" .. eq.say_link("fine weapons") .. "], then we may yet have a chance to retake the woods once more.");
	elseif(e.message:findi("acquire some information")) then
		e.self:Say("Your tracking skills are invaluable to our cause, brave " .. e.other:GetName() .. ". Our scouts have reported of a Ghoul Messenger. They have spotted him traveling between Kithicor and an undead camp in the Plains of Karana. They have attempted to track the monster and obtain any Strategic Domumentation he may be carrying. Perhaps you can succeed where others have failed. Perhaps you can earn my personal short sword.");
	elseif(e.message:findi("fine weapons")) then
		e.self:Say("My soldiers are valiant, but poorly equipped. I have heard of a magical weapon of great power, some call it the Ebony Blade. Amongst the Froglocks there can be found a Minotaur who wields this sword of might. If you are a ranger and could provide me with this sword then I would gladly exchange it with my own meager weapon. If you prefer, you may attempt to [" .. eq.say_link("acquire some information") .. "] for us instead.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 16548})) then --Handin: 1x a Strategic Map of Kithicor (16548)
		e.self:Say("Excellent work, " .. e.other:GetName() .. "!! It is obvious that you are a highly skilled tracker and warrior. Accept this reward, my personal sword which has served me well in countless battles. Perhaps you are willing to perform [" .. eq.say_link("another service") .. "] on our behalf?");
