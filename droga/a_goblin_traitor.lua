...
function event_say(e)
	...
	if  (e.other:GetFaction(e.self) < 3) then -- warmly+
		...
		elseif(qglobals["RegalBandBathezid"] == "1") then -- we handed the Signet of Service yet
			if(e.message:findi("plan")) then
				e.self:Say("Alrighty, listen listen. Skargus used to be a low rankin' nogood. He made a fake story 'bout Gragbar who was the warlord and caused him much trouble with the chief. Gragbar was cast down, chief made Skargus the warlord now. Gragbar is very angry, would take back his position and kill Skargus if he get [" .. eq.say_link("evidence") .. "] of his treachery.");
			elseif(e.message:findi("evidence")) then
				...
				e.other:SummonItem(6472); -- Traitor's note
			end
		end
	end
end
...
function event_trade(e)
	...
	if(e.other:GetFaction(e.self) < 3) then  -- need to be warmly or better
		if(item_lib.check_turn_in(e.self, e.trade, {item1 = 5728})) then -- Di'zok Signet of Service
			...
			e.self:Say("Good! They tell'd me to wait here for ya, can't say I like it but here I sits. It be good to be rid of Skargus though, so I be a happy one, oh yes. Want to know the [" .. eq.say_link("plan") .. "] then?");
			e.other:QuestReward(e.self,0,0,0,0,5728); -- Di'zok Signet of Service
