...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20648})) then
		e.self:Emote("says, 'Seek, search, turn, twist! Kazen hopes to death master truly! Read out, read in between! Talk to the pretty clouds.' Drendico turns his back to you and throws a book over his shoulder. 'Bring back book! Bring back reagents three!!' He shouts as he mumbles some more gibberish.");
		e.other:QuestReward(e.self,0,0,0,0,18086,1000);
	end
	if(item_lib.check_turn_in(e.trade, {item1 = 1278, item2 = 20656, item3 = 20655, item4 = 18086})) then
		e.self:Emote("turns to you in amazement. 'You actually did it! These are the correct components! Kazen will be most pleased. I searched the planes for almost a year and could not find all the correct components.' Drendico fiddles with the components, puts them into a box and hands it to you. 'You must have access to an army, " .. e.other:GetName() .. ". Return the components to Kazen quickly!");
