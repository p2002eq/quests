function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Welcome to my little refuge of the mind. Here you may find passage to the grand worlds within a fine book. I have few books for sale, but any good book is rare in the realm of Norrath. On occasion, I even have [" .. eq.say_link("rare finds") .. "]. Be sure to use good lighting when reading. Strain the brain, not the eye, I always say.");
	elseif(e.message:findi("rare finds")) then
		e.self:Emote("starts rummaging through a pile of books and scrolls.");
		e.self:Say("Oh yes, yes, yes. Rare books occasionally make their way to me. Ahhh!! I have one title, Before Green. Sounds like a bit of fiction. What would you like to [" .. eq.say_link("purchase") .. "]?");
	...
	elseif(e.message:findi("spectacle")) then
		e.self:Say("My spare reading spectacles were stolen by froglok raiders in the swamps. I gave chase but the little buggers hop faster than I run. They were hopping with such fury that they dropped a [" .. eq.say_link("tome binder") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10029})) then
