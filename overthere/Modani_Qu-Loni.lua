...
function event_say(e)
	...
	elseif(e.message:findi("ready")) then
		e.self:Say("Scattered throughout the world are various items. To prove your worth, go collect these and return them to me. The [" .. eq.say_link("Xolion Rod") .. "], [" .. eq.say_link("Innoruuk's Word") .. "], [" .. eq.say_link("Chalice of Kings") .. "], and [" .. eq.say_link("snow blossoms") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 10604})) then
		e.self:Say("Ah yes, Jeb's seal. The time to craft a Serpent must have come. I will need components collected to craft the first of the pieces. I will also provide you with a [" .. eq.say_link("sack") .. "] in which you must combine the items. Are you [" .. eq.say_link("ready to collect") .. "] them?");
