function event_say(e)
	...
	elseif(e.message:findi("tunics")) then
		e.self:Say("To create your tunic material you will need to combine 5 Bricks of Crude Bronze, 1 Enraged Goblin Beads, 1 Pristine Forest Drakeling Scale, 1 Golden Bandit Tooth, 1 Basilisk Tongue and 1 Goblin Parts in your assembly kit. Once you have created the proper material take it to a forge along with this mold to fashion your very own Apprentice Miners Tunic. When ye finish yer tunic come on back fer I got one more [" .. eq.say_link("assignment") .. "] for a miner that I know I can trust!");
		--Summon: Crude Breastplate Mold
		e.other:SummonItem(19637);
	elseif(e.message:findi("assignment")) then
		e.self:Say("Well da assignment is simple " .. e.other:GetName() .. ". I jess simply need ye to fetch me some items that I need to create a minin pick dat I plan on distributing to all da new miners around dese parts dat pass my exercises, much like yerself. If you would like to help me [" .. eq.say_link("collect") .. "] the items I need I could perhaps fix ye up one of dese first minin picks. Whatta ya say dere " .. e.other:GetName() .. "?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 28067, item2 = 28068, item3 = 28068})) then
		e.self:Say("Ah thank ye " .. e.other:GetName() .. ". Here is yer minin pick.");
		--Summon: Sharpened Mining Pick
		e.other:SummonItem(26078);
	end
