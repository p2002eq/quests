function event_say(e)
	if(e.message:findi("hail")) then
		if(e.other:GetFaction(e.self) < 8) then
			e.self:Say("What business do you have here?!! This here is the mine and that means if you ain't a [" .. eq.say_link("member of Miner's Guild 628") .. "], you'd best be moving on!!");
		else
	...
	elseif((e.message:findi("628"))) then
		if(e.other:GetFaction(e.self) < 5) then
			e.self:Say("Well, then, get off yer rump and give us a hand! If you don't have a mining pick, then get down there and use your fingernails!! If you're new and you want to earn a pick, you can [" .. eq.say_link("volunteer to exterminate the rats") .. "] that have been infesting the mines!!");
		end
	...
	elseif((e.message:findi("volunteer"))) then
		if(e.other:GetFaction(e.self) < 5) then
			e.self:Say("Well, someone has to do the dirty work around here. Let it be you new miners. Go patrol the mines and if you see any rats, bash them good!! Return to me with four rat pelts and I will give you some armor as payment. If you want a 628 mining cap, yer gonna have to smash that infernal [" .. eq.say_link("cleaner") .. "]!!");
		end
		if(e.other:GetFaction(e.self) < 6) then
			e.self:Say("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
		else
	...
	elseif((e.message:findi("run"))) then
		if(e.other:GetFaction(e.self) < 5) then
			e.self:Say("Well, as you most likely know, we are a non-profit organization, at least here in Kaladim. We need to make our profit elsewhere and that means a little bit of thieving. Presently we need loyal members to [" .. eq.say_link("collect the gnome profit") .. "].");
		end
		if(e.other:GetFaction(e.self) < 6) then
			e.self:Say("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
		else
	...
	elseif((e.message:findi("collect"))) then
		if(e.other:GetFaction(e.self) < 5) then
			...
			e.other:SummonItem(12164); -- Scrubber Key
		end
		if(e.other:GetFaction(e.self) < 6) then
			e.self:Say("Don't take this personally, but I can't quite trust you with such matters. Maybe a few less Butcherblock bandits would prove your worth.");
		else
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 13054, item2 = 13054, item3 = 13054, item4 = 13054})) then -- Giant Rat Pelt
		e.self:Say("It's about time you managed to smash these four!! Here. You do good work. We could use someone like you to [" .. eq.say_link("run to the gnome city") .. "].");
		e.other:SummonItem(eq.ChooseRandom(2108,2124,2118,2116,2120,2123));
	...
	elseif((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 13282}))) then -- Scrap Metal
		...
		e.other:SummonItem(12165); -- Miners Cap 628
		e.other:Faction(220, 10); -- Miners Guild 628
		e.other:Faction(33, -10); -- Circle Of Unseen Hands
		e.other:Faction(29, -10); -- Butcherblock Bandits
		e.other:Faction(77, 10);  -- Deeppockets
	...
	elseif((e.other:GetFaction(e.self) < 5) and (item_lib.check_turn_in(e.self, e.trade, {item1 = 12167}))) then -- Gnome Take (Bad)
		...
		e.other:SummonItem(eq.ChooseRandom(10009, 10015, 10500));
		e.other:Faction(220, 10); -- Miners Guild 628
		e.other:Faction(33, -10); -- Circle Of Unseen Hands
		e.other:Faction(29, -10); -- Butcherblock Bandits
		e.other:Faction(77, 10);  -- Deeppockets
