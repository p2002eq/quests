function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail dere me friend. I be Haldorak Foamymugs High Priest of Clerics dat heed the call of Brell. I am a very busy Dwarf for I have many new house recruits to train. Ye see I am in charge of all the young ones that pass through our halls. They must all undergo and pass me [" .. eq.say_link("tests") .. "] before they are truly ready to travel the dangerous world of Norrath.");
	elseif(e.message:findi("tests")) then
		e.self:Say("Well ye see " .. e.other:GetName() .. ", we cant just send anyone out into dis world to spread Brells word and not make sure dey are properly trained in the ways of battle. Dis is why I have created some exercises for our young ones so that dey may be able to do which will in the end enable them to make a full set of their own armor. If`n ye are a young [" .. eq.say_link("cleric") .. "] of Brell well den these special exercise will surely be right for ye! Aye.");
	elseif(e.message:findi("cleric")) then
		e.self:Say("Well den lets not waste anytime and lets get you started! Here is yer Assembly Kit of Brell. In this kit ye will be collected many different items from both da Mountains of Butcherblock as well is in our great minin city o Kaladim! You will then place certain items in yer kit to create armor materials that you will then combine in a forge with an armor mold to make yer armor piece. If`n yer ready to begin craftin yer armor then I will be glad to present you with da material [" .. eq.say_link("recipes") .. "]!");
	elseif(e.message:findi("recipes")) then
		e.self:Say("Now yer talking " .. e.other:GetName() .. ", I be glad to see yer ready to begin crafting yer honor and heed yer calling as a servant o Brell! I have the armor material recipes and molds for Brells Blessed Platemail [" .. eq.say_link("Helm") .. "], [" .. eq.say_link("Bracer") .. "], [" .. eq.say_link("Armguards") .. "], [" .. eq.say_link("Boots") .. "], [" .. eq.say_link("Greaves") .. "], [" .. eq.say_link("Gauntlets") .. "], and [" .. eq.say_link("Breastplate") .. "]. All ye must do is simply tell me what armor piece ye want to craft and I will gladly present ye with the material recipe and armor mold necessary fer craftin it.");
		-- Holy Assembly Kit
