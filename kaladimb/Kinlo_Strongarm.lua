...
function event_say(e)--indifferent or better
	...
	elseif(e.message:findi("ancient longsword") and e.other:GetFaction(e.self) < 6) then
		e.self:Say("The ancient smiths, for what I think yer askin' about, ain't been around fer a long time, laddy. Last anyone heard o' the ancients was 'fore my time even. There be more than a few [" .. eq.say_link("tales") .. "] about 'em.");
	elseif(e.message:findi("tales") and e.other:GetFaction(e.self) < 6) then
		e.self:Say("There be some who talk about how the ancient smiths went down to work fer the Duke o' Below, sayin' he be needin' some fine dwarven smiths fer makin' some castle or some such. There be tales that the ancients were so good at smithin' that the Duke took 'em down and made 'em head smiths over his minions. There's even tales that some o' the smiths' souls were [" .. eq.say_link("trapped in their hammers") .. "], stuck there by some necromancer or somethin', as punishment or retribution.");
	elseif(e.message:findi("trapped in their hammers") and e.other:GetFaction(e.self) < 6) then
		e.self:Say("Aye. Some says their souls were bound into their hammers by an Erudite [" .. eq.say_link("necromancer and enchanter") .. "]. They say they used the power of the hammers ta profit by makin' ships, boats, and houses better than any other merchants. Sad tale, if'n it be true.");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20478}) and e.other:GetFaction(e.self) < 6) then
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20485}) and e.other:GetFaction(e.self) < 6) then
		...
		eq.spawn2(67089,0,0,-189.5,346.8,1.8,69);
