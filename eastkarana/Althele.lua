...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20448})) then
		...
		if (not eq.get_entity_list():IsMobSpawnedByNpcTypeID(15042)) then eq.get_entity_list():GetSpawnByID(329482):Repop(); end -- repop Fang if he is not up
		if (not eq.get_entity_list():IsMobSpawnedByNpcTypeID(15043)) then eq.get_entity_list():GetSpawnByID(329483):Repop(); end -- repop Tholris if he is not up
...
function event_timer(e)
	...
	elseif(e.timer == "emotes") then
		...
		eq.signal(15178,99,0);
		eq.signal(15167,99,3000);
		eq.signal(15170,99,6000);
		eq.signal(15043,99,9000);
