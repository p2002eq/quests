...
function event_waypoint_arrive(e)
	...
	elseif e.wp == 106 and not construct then
		construct = true;		
		eq.spawn2(204460,0,0,-4550,5118,-5,128) 	-- #a_construct_of_nightmares (Final Encounter)
	elseif e.wp == 109 and not dialogue then
		...
		eq.signal(204065,20,3*1000);
	end
	
	----DEBUG----------
	--if e.wp == 2 then
	--	e.self:GMMove(-4549,5350,4,-1);
...
function event_trade(e)
    ...
    if construct_dead and not hand_in and item_lib.check_turn_in(e.self, e.trade, {item1 = 9258}) then  
		...
		eq.spawn2(204065,0,0,-4544,4943,30,254);	--spawn Terris Thule
		hand_in = true;
		eq.resume();
	end	
