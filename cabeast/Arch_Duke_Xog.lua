...
function event_enter(e)
	if(e.other:HasItem(18205)) then -- Guild Summons
...
function event_say(e)
	...
	elseif(e.message:findi("greenmist")) then
		e.self:Emote("appears surprised at your words. 'You know of Greenmist? The Unholy Khukri of Rile? We once had knowledge of this weapon, but when our great cities were destroyed in 1056 A.G., so, too, were all the great libraries. We lost all records. An explorer named [" .. eq.say_link("Argest") .. "] claimed to have found one library still intact. Pure babble.'");
	elseif(e.message:findi("argest")) then
		e.self:Say("Once a Lord of Pain, Argest is now a great explorer. No lizard has seen more of Kunark than he. He returned one season ago to tell tales of an [" .. eq.say_link("ancient library") .. "]. He said that he believed that there he would find a tome which would reveal the location of the ancient crusader weapon, Greenmist.");
	elseif(e.message:findi("ancient library")) then
		e.self:Say("There are many ancient libraries yet to be discovered. Our once great cities have been decimated, if not by our foes, then by nature itself. Within the outlands are many ruins which have yet to reveal themselves. We look forward to the discovery of these ruins by such explorers as Lord Argest the Great. If only we knew [" .. eq.say_link("where") .. "] he was...");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 18205})) then -- Guild Summons
		...
		e.other:QuestReward(e.self,0,0,0,0,5120,100); -- Pawns Khukri
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 18051, item2 = 5126})) then -- Stupendous Tome and Heros Khukri
		...
		e.other:Faction(62,20); -- Crusader of Greenmist
		e.other:Faction(193,10); -- Legion of Cabilis
		e.other:QuestReward(e.self,0,0,0,4,5128,4000); -- Lord of Pains Khukri
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 14791})) then -- Illegible Note: Bracer
		e.self:Emote("takes the note and glances at it briefly then lets out a heavy sigh and stares out over the city for several minutes. Finally he says quietly, 'Several months ago we lost contact with one of our crusaders. His name is, or was, Geanik. The last we heard from him he was furthering our cause against the loathsome Goblins that reside in Warsliks Wood. Just recently we were visited by a [" .. eq.say_link("strange Iksar") .. "].'");
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 14807, item2 = 10034, item3 = 10034})) then -- Traitors Heart and 2x Sapphire
		e.self:Say("Thanks for resolving this issue. Here is your reward");
		e.other:QuestReward(e.self,0,0,0,0,14808,10000); -- Xog's Reference: Bracer
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 14792})) then -- Illegible Note: Gauntlets
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 14806, item2 = 10032, item3 = 10032})) then -- Head of Blackhand and 2x Star Ruby
		e.self:Say("Well done, " .. e.other:GetName() .. ".");
		e.other:QuestReward(e.self,0,0,0,0,14809,10000); -- Xog's Reference: Gauntlet
	end
