...
function event_say(e)
	...
	elseif(e.message:findi("militia pike")) then
		e.self:Say("The pike is the prime weapon of Cabilis warriors. It can be upgraded, too, such as from the partisan pike to the militia pike and beyond. All these pikes may be slung on one's back when your hands are needed for other pursuits. To upgrade a pike is something that is learned, but it can never be performed without a [" .. eq.say_link("geozite tool") .. "]. Do you [" .. eq.say_link("desire to upgrade the partisan pike") .. "]?");
	...
	elseif(e.message:findi("what geozite tool")) then
		e.self:Say("The Geozite Tool is used to sharpen the pikes of the Legion of Cabilis. Only it can produce the serrated edges necessary for these deadly weapons. They are not handed out to just any broodling. The tool is only given to warriors who serve the legion. Do you [" .. eq.say_link("want a geozite tool") .. "]?");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 12675, item2 = 12677, item3 = 5130})) then -- Froglok Escort Fife, An Iksar Head and Partisans Pike
		e.self:Say("You have perfomed just as expected. I bestow upon you the rank of militiaman. Here is your new pike. Past this, you shall require the [" .. eq.say_link("want geozite tool",false,"geozite tool") .. "] to upgrade your future pikes and mancatchers. We see much promise in you, militiaman. Go forth to serve the realm.");
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 12658})) then -- Full Pincer Satchel
		e.self:Say("You are a true warrior of Cabilis. You obviously are aware that in order to upgrade your pike you shall need a [" .. eq.say_link("what geozite tool",false,"geozite tool") .. "]. Take this note to the Lord of the outer gates. He desires a young warrior for a small task. Do this and he is instructed to reward you with the tool.");
		e.other:Faction(30,2); -- Cabilis Residents
		e.other:Faction(193,2); -- Legion of Cabilis
		e.other:Faction(282,2); -- Scaled Mystics
		e.other:Faction(317,2); -- Swift Tails
