...
function event_say(e)
	...
	elseif(e.message:findi("samples") and research == 1) then
		e.self:Say("Oh, some antidote ingredients that I stumbled upon not long ago. They should meet his needs. Will you be so kind as to [take the antidote ingredients] to Zimloro?");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 4758})) then -- Zimloro Ingredient List
		e.self:Say("Ah! Master Zimloro sent you, eh? This note indicates that he wants some specific ingredients. Luckily, I've got several [fresh samples] that I've only recently collected.");
		research = 1;
