...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Greetings, Lenins. Watch where you step for the [" .. eq.say_link("needle clawed hoppers") .. "] are just about everywhere!");
	elseif(e.message:findi("needle clawed hoppers")) then
		e.self:Say("Those wretched vermin swarm to our city and trade routes in search of the refuse left by careless travelers who pass through these parts. They are just about useless and their stench brings bile to my throat. I would have set out to poison the whole lot of them if it was not for their unique [" .. eq.say_link("claws") .. "].");
	elseif(e.message:findi("claws")) then
		e.self:Say("The claws of the hoppers act as a [" .. eq.say_link("spirit anchor") .. "]. They are specialized in tearing into the fiber of the shadow beings that roam the thicket. I find these claws useful in binding the silk fashioned by the [" .. eq.say_link("shadeweavers") .. "].");
	elseif(e.message:findi("shadeweavers")) then
		e.self:Say("The shadeweavers hunt the beings of shadow found within the thicket. From them they fashion their shadow armor they call Xakra. We have tried to use this strong and lightweight armor in the past, but the shadows just disperse. I have found that sewing this armor onto a suitable [" .. eq.say_link("spirit anchor") .. "] will keep it from dispersing.");
	elseif(e.message:findi("spirit anchor")) then
		e.self:Say("A spirit can only materialize within our realm by binding themselves to an object that is a spirit anchor. Some bones and stones have such properties. A spirit anchor can also do the most damage to the material form of such spirits when used as a weapon. I use the claws of the hoppers as a sewing needle of sorts to [" .. eq.say_link("bind the silk") .. "] made by the [" .. eq.say_link("shadeweavers") .. "]. A standard sewing needle would only pass through the substance of the silk. The hopper claws can pierce through their ethereal fabric effectively.");
