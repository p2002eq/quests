function event_say(e)
	...
	if(e.message:findi("clothing")) then
	...
	if(e.message:findi("important research")) then
	...
	if(e.message:findi("new information")) then
	...
	if(e.message:findi("wristband")) then
	...
	if(e.message:findi("pantaloons")) then
	...
	if(e.message:findi("robe")) then
		...
		e.other:SummonItem("11395","1"); end
	end
end
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20358, item2 = 20356, item3 = 20402})) then
		e.self:Emote("fashions a grip out of the Giant Briar Snake Skin, fastens the Petrified Eyeball to a steel mount on one end of the staff, and tarnishes the wood of the staff with a shimmering black substance.");
		e.self:Say("Here is your new Abattoirs Initiate Staff, " .. e.other:GetName() .. ". May it serve you well in your pursuit for power.");
		e.other:SummonItem(20418);
		e.other:Faction(143, 1);
		e.other:Faction(79, -3);
		e.other:Faction(112, -3);
		e.other:Faction(56, -3);
