function event_say(e)
	...
	elseif(e.message:findi("audience with the overlord")) then
		e.self:Say("The Overlord is not seeing anyone at this time. however. I may have work for you. You seem to have some experience with the kobold annoyance in the region. Are you [" .. eq.say_link("interested") .. "]. " .. e.other:GetName() .. "?");
	elseif(e.message:findi("interested")) then
		e.self:Say("Then I shall give you a [" .. eq.say_link("task") .. "]. I assume you are aware of the kobold lair nearby. Many of our adventurous knights and priests crusade to destroy those pests so seeing many Erudites frequent the area is normal. However. it is not normal to see someone carry large crates into the lair and return empty handed. This person's movements also show he is attempting to remain unseen.....the [" .. eq.say_link("fool") .. "].");
	...
	elseif(e.message:findi("task")) then
		e.self:Say("According to the evidence we've uncovered, it appears the kobolds are nothing but pawns to harass us. Or perhaps even all of Odus. If they wish to play these games, then we'll humor them. We'll take their pawn with one of our knights. Will you be the [" .. eq.say_link("knight") .. "], " .. e.other:GetName() .. "?");
...
function event_trade(e)
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1776, item2 = 1777, item3 = 18174, item4 = 1778})) then--crate of ore,couriers head,suppliers head,sealed letter
		...
		e.other:Faction(112,-20); -- -Gate Callers
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 1892})) then-- Empty Jar
		...
		e.other:Faction(112,-40); -- -Gate Callers
		e.other:Faction(56,-40);  -- -Craftkeepers
		e.other:Faction(60,-40);  -- -Crimson Hands
