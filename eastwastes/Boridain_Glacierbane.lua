...
function event_trade(e)
    ...
    if (item_lib.check_turn_in(e.self, e.trade, {item1 = 30265})) then -- Dull Bladed Axe
		...
		eq.start(370);
	elseif route_end and item_lib.check_turn_in(e.self, e.trade, {item1 = 30266}) then -- Rabid Kodiak Skin
		...
		e.other:QuestReward(e.self,0,0,0,0,30267,1000); -- Broken Axe
		eq.depop_with_timer();
	end
...
function event_waypoint_arrive(e)
	...
	elseif e.wp == 18 and not route_end then
		...
		e.self:SetAppearance(1);
		eq.spawn2(116597, 371, 0, 1559, -2304, 313,251.0); -- Rabid Tundra Kodiak
	end
end
