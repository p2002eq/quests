...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Go away! I'm busy! I don't have time for scoundrels like you! Leave me alone or I shall, um..turn you into, er, a fish or something! That is what us powerful wizards do to those who annoy us! Yes, that is it. A fish! Did that [" .. eq.say_link("that was scary",false,"sound scary") .. "] enough?");
	...
	elseif(e.message:findi("translate")) then
		e.self:Say("Ah, codes are my specialty! It's what I did for the Circle before Hanns took over. But the Fox is wrong. I owe him nothing! As a fact, he owes me! He wants this translated - he will have to [" .. eq.say_link("pay") .. "]!");
...
function event_trade(e)
	local item_lib = require("items");
		...
		e.self:Say("Hmm, interesting. This document is not only encoded, but written in a very obscure language. From what I can gather, it's a variant of elder Teir'Dal, but not one I've encountered. I can not fully [" .. eq.say_link("translate") .. "] this, but I know one who can. Find Yendar and give him this.");
		e.other:SummonItem(28053);
		e.other:Faction(240,20);--Order of Three
		e.other:Faction(21,-10);--Bloood Sabers
