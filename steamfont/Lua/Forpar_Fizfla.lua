...
function event_say(e)
	...
	if(e.message:findi("mystical instrument")) then
	...
	if(e.message:findi("components")) then
	...
	if(e.message:findi("next pieces")) then
	...
	if(e.message:findi("one more to go")) then
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.trade, {item1 = 20378})) then --Note to Forpar Fizfla
		...
		e.other:SummonItem(20380); --Forpar's Note to Himself
	...
	elseif(item_lib.check_turn_in(e.trade, {item1 = 20380, item2 = 20524, item3 = 20525, item4 = 5520})) then --Forpar's Note to Himself, Kedge Backbone, Petrified Werewolf Skull, Amygdalan Tendril
		e.self:Say("Wow! I didn't think a " .. e.other:Race() .. " like you would be able to gather all of those things.");
		e.self:Emote("rambles around and works with small tools for a good ten minutes before handing you a very fine looking lute head.");
		e.other:SummonItem(20535); --Mystical Lute Head
		e.self:Say("Now, do you want to gather the next pieces for me?");
	
	elseif(item_lib.check_turn_in(e.trade, {item1 = 11602, item2 = 11622, item3 = 16905, item4 = 5520})) then --White Dragon Scales, Red Dragon Scales, Metal Bits
		...
		e.other:SummonItem(20536); --Mystical Lute Body
	
	elseif(item_lib.check_turn_in(e.trade, {item1 = 20535, item2 = 20536, item3 = 20526, item4 = 5520})) then --Mystical Lute Head, Mystical Lute Body, Undead Dragongut Strings
		e.self:Say("Goodness! I can hardly contain myself! This will be my greatest creation ever!");
		e.self:Emote("sits down, slides the lute head into place and screws it together. After a few minutes, he begins to string the lute with the gut strings of the poison dragon. A wonderful sound fills the air as he strums the lute once and hands it to you.");
		e.self:Say("I hope you find a good use for that! I bet you could play it from the mountaintops and the people below would hear you. Thank you, " .. e.other:GetName() .. ", for making my lifelong dream come true!");
		e.other:SummonItem(20538); --Mystical Lute
	end
