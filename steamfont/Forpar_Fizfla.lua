function event_say(e)
	...
	elseif(e.message:findi("mystical instrument")) then
		e.self:Say("I have made many mystical instruments in the past. Each one seemed to be better then the last. I have always had a dream of building the world's best lute, but I have yet to find someone brave enough to gather the [" .. eq.say_link("components") .. "] needed to make the instrument.");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 20378})) then --Note to Forpar Fizfla
		e.self:Say("So you know Vedico! How is the lass? If Vedico would give you the time of day, you must be something special. What can I do for you? I hope you have not come to have me make another [" .. eq.say_link("mystical instrument") .. "].");
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 20380, item2 = 20524, item3 = 20525, item4 = 5520})) then --Forpar's Note to Himself, Kedge Backbone, Petrified Werewolf Skull, Amygdalan Tendril
		e.self:Say("Wow! I didn't think a " .. e.other:Race() .. " like you would be able to gather all of those things. Forpar rambles around and works with small tools for a good ten minutes before handing you a very fine looking lute head. 'Now, do you want to gather the [" .. eq.say_link("next pieces") .. "] for me?");
	...
	elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 11602, item2 = 11622, item3 = 16905})) then --White Dragon Scales, Red Dragon Scales, Metal Bits
		e.self:Say("Wooooooo! You are doing a wonderful job, " .. e.other:GetName() .. ". I wish I could go out and gather these things myself.' Forpar sits back down at his desk and pulls several very strange looking tools out. Eventually he looks up at you and says, 'The body is done! Only [" .. eq.say_link("one more to go",false,"one more") .. "] piece to go!");
