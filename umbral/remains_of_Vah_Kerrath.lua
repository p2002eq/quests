...
function event_say(e)
    ...
    elseif(e.message:findi("worthy")) then
        if(fac <= 2) then
            e.self:Say("The squishy mortal has proven itself by releasing Elysian spirits. We greet the Elysian spirits to our realm and rejoice in their freedom. Go on fleshling, ask the bone man to tell you of [" .. eq.say_link("Vex Thal") .. "].");
        else
            e.self:Say("Begone from my sight corruptor of spirits. I will only aid my allies.");
        end
    elseif(e.message:findi("vex thal")) then
        if(fac <= 2) then
            e.self:Say("The city of shadow is sealed by powerful magic. Only one that wears the talisman of Akelha Ra can pass through the seals on the gate. I have not been beyond the gates but rumors of [" .. eq.say_link("dark prophets") .. "] that possess great power have been heard throughout the ages.");
        else
...
function event_trade(e)
    ...
    elseif(item_lib.check_turn_in(e.self, e.trade, {item1 = 28102, item2 = 28102, item3 = 28102, item4 = 28102})) then -- 4x Elysian Skull
        e.self:Say("You have done well. Clear your mind of all distractions and ask the whisperlings if you are [" .. eq.say_link("worthy") .. "] of my knowledge.");
