...
function event_say(e)
    ...
    if(fac <= 5) then
        if(e.message:findi("hail")) then
            e.self:Say("Hello, " .. e.other:GetName() .. ". My guards seem to find you harmless enough, but I have yet to determine if you are trustworthy. We are always on the lookout for additional hands to assist in our work. Perhaps you are... sympathetic to our [" .. eq.say_link("situation") .. "].");
        elseif(e.message:findi("situation")) then
            e.self:Say("Those blasted Houses in Shadow Haven. They think they have the right to own all trade going through the Haven. Well it's about time they learn that trade will continue with or without their consent. I know a few people on the Trade Commission who understand the way things really [" .. eq.say_link("work") .. "] and they don't care who does the trading as long as they get their cut.");
        elseif(e.message:findi("work")) then
            e.self:Say("Excellent! We like to keep changing who we use for deliveries. Keeps those dumb Defenders guessing and reduces the chances of goods not reaching their [" .. eq.say_link("destination") .. "].");
