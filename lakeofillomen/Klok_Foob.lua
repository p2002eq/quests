function event_say(e)
	if(e.message:findi("hail")) then
		...
		e.self:Say("Buy, buy, buy. Go ahead, make a purchase. I am not here to talk. Go to Cabilis for talk. Come to Foob for armor. Now hurry and.. Ooh.. Foob is starvings. Foob wish he had his [" .. eq.say_link("munch break favorite") .. "].");
	elseif(e.message:findi("munch break favorite")) then
		-- Catfish Croak Sandwich [Part 2]
		e.self:Emote("rubs his tummy. 'Oh!! Catfish croak sandwich is my favorite, it is. Can only eats on my break. I haves one and one is what the Haggle Baron says to Foob. Only thing is Foob gets way tired from standing. Foob needs some broodling to [" .. eq.say_link("fetch the catfish croak sandwich") .. "] for him.'");
	elseif(e.message:findi("fetch.*sandwich")) then
		-- Catfish Croak Sandwich [Part 3]
		e.self:Say("What?!! You dus gets Foob his sandwich for his munch break!! You be a good tail. Gets me two sandwiches, one fer break and one fer work, then me pay you something for the troubles. But it be no trouble. Foob is sure you know [" .. eq.say_link("how to make sandwich") .. "].");
...
function event_trade(e)
	...
	if(item_lib.check_turn_in(e.self, e.trade, {item1 = 12451, item2 = 12451})) then
		-- Catfish Croak Sandwich [" .. eq.say_link("Part 8") .. "][End]
