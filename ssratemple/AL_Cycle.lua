...
function event_spawn(e)
	...
	if (qglobals["AL_Cycle_Rhag2"] == "1" and eq.get_entity_list():IsMobSpawnedByNpcTypeID(162192) == false) then  --checks for Rhag`Zhezum death and verifies Rhag`Mozdezh is not already up
	...
	elseif (qglobals["AL_Cycle_ArchLich"] == "1" and eq.get_entity_list():IsMobSpawnedByNpcTypeID(162517) == false) then  --checks for Rhag`Mozdezh death and verifies Rhag`Mozdezh is not already up
