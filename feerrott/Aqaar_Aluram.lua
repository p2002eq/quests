...
function event_say(e)
	if(e.message:findi("hail")) then
		e.self:Say("Hail traveler. may I assist you with something. or are you just [" .. eq.say_link("passing through") .. "]?");
	elseif(e.message:findi("passing through")) then
		e.self:Say("Very well friend. please take care when passing beyond the river to the east.  Several researchers and their mercenary escort passed through our camp en route to the [" .. eq.say_link("temple") .. "].  That was last week and they have yet to return.  We are holding off on our [" .. eq.say_link("expedition") .. "] until we can learn more about what happened to them.");
	...
	elseif(e.message:findi("expedition")) then
		e.self:Say("Several weeks ago, I received the news from M'lady Morkul that the Academy was researching this temple. It had been years since I last visited this site, so naturally I was interested. Of course, that was before we set camp in this accursed place. I must be honest; I was quite ready to leave after our [" .. eq.say_link("first night") .. "] here.");
	elseif(e.message:findi("first night")) then
		e.self:Say("After securing the site, we set sentinels to alert us to any patrols or prying jungle eyes. Unfortunately, our magic did little to provide us with a night's rest that eve. The incessant sound of drums and the screams from within the temple had little regard for our lack of rest and already nervous disposition. Several times during the course of my slumber that night, I was taunted by foul [" .. eq.say_link("visions") .. "] of an unnatural sort.");
	elseif(e.message:findi("visions")) then
		e.self:Say("My mind was filled with images of grotesque rituals. The lizards that inhabit that place were gathered around magical portals, chanting in a tongue too wretched to recall. As they spoke and danced, the denizens of some horrid void were pouring from the shimmering rifts! I awoke to find the [" .. eq.say_link("others") .. "] in the same state of horror! When I gathered my senses, I noticed that all of our equipment has been covered in some sort of [" .. eq.say_link("black ooze") .. "], though our sentinels displayed no signs of having been triggered.");
	elseif(e.message:findi("black ooze")) then
