...
function event_signal(e)
   ...
   elseif (e.signal == 9) then 
		...
		if ( mobs_killed == 16 ) then 
			eq.spawn2(201498, 0,0, -122, -1134, 74,508.0);
		end
   end
...
function spawn_mobs(wave)
	...
	if ( eq.get_entity_list():IsMobSpawnedByNpcTypeID(201494) == false ) then 
		eq.spawn2( 201494, 0, 0, -177, -1188, 73,2.0);
		eq.spawn2( 201494, 0, 0,  -82, -1188, 73,2.0);
	end
   ...
   if (wave < 5) then
		...
		for i = 1, 4, 1 do
			...
			if (spawn_at == 1) then
				npc = eq.spawn2( eq.ChooseRandom( 201495, 201496, 201497, 201499 ), 52, 0,-260.0, -1233, 73, 0);
			elseif (spawn_at == 2) then
				npc = eq.spawn2( eq.ChooseRandom( 201495, 201496, 201497, 201499 ), 53, 0,-482.0, -1120, 73, 64);
			elseif (spawn_at == 3) then
				npc = eq.spawn2( eq.ChooseRandom( 201495, 201496, 201497, 201499 ), 54, 0,-42.0,  -1121, 73, 190);
			end
			if (npc.valid) then 
...
function spawn_prisoners()
   ...
   eq.spawn2(201493, 0, 0, -88, -1046, 73,250.0);
   eq.spawn2(201493, 0, 0, -129, -1046, 73,250.0);
   eq.spawn2(201493, 0, 0, -171, -1046, 73,250.0);
end
