...
function event_say(e)
	...
	elseif (e.message:findi("prepared")) then
		e.self:Emote("nods slightly.  'Very well. When you are ready, you may [" .. eq.say_link("I am ready to begin the trial of stoning", false, "begin the trial of stoning") .. "]. The archers will not cease their punishment until nothing else stands.  Do not let the prisoners take more than they can bear.  We shall judge the mark of your success.'");
	elseif (e.message:findi("ready to begin the trial of stoning")) then
		if ( stoning_flag == 0 ) then 
			...
			eq.spawn2(201505, 0, 0, trial_x, trial_y, trial_z, trial_h); -- #Event_Stoning_Control (201505)
...
function event_timer(e)
	...
	elseif (e.timer == "proximitycheck") then
		...
		if ( ProximityCheck(trial_x, trial_y, trial_z, 250) == false) then 
			eq.stop_timer(e.timer);
			eq.signal(201505,2);	--signal event fail to #Event_Stoning_Control (201505)
