...
local trial_z           = 75;
local trial_h           = 155;
local trial_mobs		= {201464,201465,201466,201467,201470 };
...
function event_say(e)
	...
	elseif (e.message:findi("prepared")) then
		e.self:Emote("nods slightly.  'Very well. When you are ready, you may [" .. eq.say_link("I am ready to begin the trial of execution", false, "begin the trial of execution") .. "]. The victim will perish should the hooded executioner reach him.  Its life will end only when all of the nemeses which accompany it also perish.  We shall judge the mark of your success.'");
	elseif (e.message:findi("ready to begin the trial of execution")) then
		if ( execution_flag == 0 ) then 
			...
			eq.spawn2(201471, 0, 0, trial_x, trial_y, trial_z, trial_h); -- #Event_Execution_Control (201471)
...
function event_timer(e)
	...
	elseif (e.timer == "proximitycheck") then
		...
		if ( ProximityCheck(trial_x, trial_y, trial_z, 250) == false) then 
			eq.stop_timer(e.timer);
			eq.signal(201471,2);	--signal event fail to #Event_Execution_Control (201471)
